...
\includegraphics[height=.3\textheight]{figures/12_Definiteness}
...
\gll	\textbf{Guld} \textbf{ið} ir  dyrt.  \\
		\textbf{gold.{\def}} be.{\prs}.{\sg}  expensive.{\n}  \\
...
\ea
	...
	\gll	\bfseries FôggLân  skâ  en  fôll  int  mat…\\
			\bfseries bird.{\pl}.{\def}  shall.{\prs}  one  {\prag}  {\neg}  feed\\
	}
	\ex {
...
\gll 	\textbf{Sockerkak} ä  dä  bäst  Kâtt’n  vet.\\
		\textbf{Sponge\_cake} be.{\prs}  {\def}.{\n}  best  cat.{\def}  know.{\prs}\\
...
\gll 	He  kall  ve  fö  \textbf{sjanostn} för  gammalt.\\
		it  call.{\prs}  we  for  \textbf{sand\_cheese.{\def}} for  old.{\n}\\
...
shall.{\prs}.1{\pl}  have.{\inf}  \textbf{curdled\_milk.{\def}} to  dinner.{\gen}\footnotemark{} \\
...
want.{\prs}  you  have.{\inf}  \textbf{snuff.{\def}} with  PRO.1{\sg}.{\obl}\\
...
so  shall.{\prs}  I  warm.{\inf}  \textbf{milk.{\def}} for  PRO.3{\sg}.{\m}.{\dat}\\
\glt ‘As soon as I have got the wood bundle into the house, I’ll warm some milk for him.’ (Cat Corpus)
...
\ea 
	...
	\gll 	så  skari  wärm  \textbf{mjötje} a  num.\\
			so  shall.{\prs}\_I  warm.{\inf}  \textbf{milk.{\def}} for  PRO.3{\sg}.{\m}.{\dat}\\
	}
	\ex {
	\langinfo{Våmhus (Os)}{}{}\\
	...
	\gll 	so  ska  i  werm  \textbf{miö:ts\={i}} a  na.\\
			so  shall.{\prs}  I  warm.{\inf}  \textbf{milk.{\def}} for  PRO.3{\sg}.{\f}.{\dat}\\
	\glt  	‘As soon as I have got the wood bundle into the house, I’ll warm some milk for him.’ (Cat Corpus)
	}
	\z 
\z
...
However, the same book also lists expressions such as \textstyleLinguisticExample{res päroni} ‘peel potatoes.{\def}.{\pl}’ (\citet[176]{AnderssonEtAl1999}). In the Cat Corpus, there is at least one clear case of a definite form:
...
it  take.{\pst}  {{\inf}m}  blow.{\inf}  \textbf{southerly\_wind.{\def}}\\
...
and  make.{\pst}  up  fire.{\def}  for  {{\inf}m}  roast.{\inf}  {\refl}  {\indf}  coffee\_drop\\
...
but  they  dare.{\pst}  {\neg}  put.{\inf}  \textbf{fire.{\def}} on   that  \\
...
we  {\prog}.{\prs}.1{\pl}  get.{\prs}.1{\pl}  little  about  \textbf{milk.{\def}}\\
...
A similar example is found in \citet{Näsman1733}. It contains a definite form \textstyleLinguisticExample{Snustobakin} ‘snuff-tobacco.{\def}’ which corresponds to an indefinite form in the accompanying Swedish translation (b), making the intended interpretation fairly clear:
...
\ea\label{}
	...
	\gll 	medan  Præstn  hölt  pâ  at  predika.\\
			while  clergyman.{\def}  keep.{\pst}  on   {{\inf}m}  preach.{\inf}\\
...
he  have.{\prs}  {\neg}  \textbf{any} \textbf{money.{\pl}.{\def}}\\
...
and  {\neg}  dare.{\pst}  I  go.{\inf}  up  to  farm.{\def}.{\pl}  either\\
...
it  be.{\prs}  {\neg}  \textbf{any} \textbf{snow.{\def}} on  ground.{\def}\\
...
he  ask.{\pst}  \textbf{which.{\dat}.{\sg}.{\m}} \textbf{god.{\def}.{\dat}.{\sg}} that.{\n}  sign.{\def}  belong.{\pst}\\
...
{\neg}  carry.{\inf}  \textbf{much} \textbf{food.{\def}} with  {\refl}\\
...
\gll De  snakket  ikke   \textbf{mange} \textbf{  ordene.}\\ % {\href{http://kh.hd.uib.no/cgi-dos/roman-bm.bat?P55377C00#here}{ikke}}
		they  talk.{\pst}  {\neg}  \textbf{many} \textbf{word.{\pl}.{\def}}\\
...
it  take.{\pst}  {\neg}  \textbf{many} \textbf{second.{\pl}.{\def}}\\
...
we  have.{\prs}.1{\pl}  \textbf{{\def}} \textbf{telephone}\\
...
but  it  go.{\prs}  {\prag}  {\neg}  well  {{\inf}m}  have.{\inf}  \textbf{cat.{\def}}\\
...
we  have.{\pst}  {\prag}  {\neg}  \textbf{iron\_stove.{\def}} so\\
\gll en  kodd  ju  åt  b\textit{a}ka  bulla….\\
one  can.{\pst}  {\prag}  {\neg}  bake.{\inf}  bun.{\def}.{\pl}\\
...
and  for long  ago  where they  {\neg}  have.{\pst}  \textbf{meat-grinder.{\def}}\\
...
this  do.{\prs}  {\neg}  for  one  that  have.{\prs}  \textbf{fly.{\def}} \\
...
we  have.{\pst}.1{\pl}  \textbf{horse.{\def}} when  I   be.{\pst}  small\\
\glt ‘We had a horse when I was a kid.’ (questionnaire)
...
see  how  they  have.{\prs}.3{\pl}  write.{\sup}  \textbf{with} \textbf{knife.{\def}.{\dat}}\\
...
now  begin.{\pst}  here  {{\inf}m}  become.{\inf}  \textbf{evening.{\def}} \\
...
it  hold.{\pst}  on  {{\inf}m}  become.{\inf}  \textbf{spring.{\def}} now\\
...
collect.PP  {\refl}.3{\pl}  around  snake.{\def}  and  woman.{\def}\\
...
therefore  can.{\prs}  one  {\neg}  take.{\inf}  \textbf{ointment.{\def}} every  day  \\
...
when  one  have.{\prs}  \textbf{time.{\def}} {{\inf}m}  read  \\
...
father  drink.{\prs}.3{\sg}  \textbf{{\def}} \textbf{coffee} every  morning\\
...
\ea 
	...
	\gll	 Aju  manciatu  tanti  voti  \textbf{u} \textbf{piscispata,}\\
			have.{\prs}.1{\sg}  eat.PP  many  time.{\pl}  \textbf{{\def}} \textbf{swordfish}\\
	...
	\gll 	Ho  mangiato  tante  volte  \textbf{il} \textbf{pesce} \textbf{spada,}\\
			have.{\prs}.1{\sg}  eat.PP  many  time.{\pl}  \textbf{{\def}} \textbf{fish} \textbf{sword}\\
...
he  smell.{\prs}.3{\sg}  \textbf{{\def}} \textbf{vodka}\\
...
is  attacked  by\_{\def}.{\pl}  hacker\\
...
in  {\def}  basement  it  there  have  \textbf{{\def}}  \textbf{P{\art}{\art}} \textbf{mouse.{\pl}}\\
\gll et  dans  l’  évier  vivent  ?les  /  des  cafards.\\
and  in  {\def}  sink  live.{\prs}.3{\pl}  {\def}    P{\art}{\art}  cockroaches\\
