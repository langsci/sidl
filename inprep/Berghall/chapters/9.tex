...
In the example above one of the topics is in the singular, the other in the plural. But in \REF{ex:3:x357}, repeated below as \REF{ex:9:x1920}, there are two third person singular topics alternating, with only the verbal marking to indicate who is doing what:
...
The main placement for the contrastive focus marking is the subject \REF{ex:9:x1688},\footnote{Waskia has an identical morpheme \textit{ke}, labeled as a subject marker, which is very similar in function \citep[36]{RossEtAl1978}%Paol
.} \REF{ex:9:x1689}, or the non-verbal predicate of a verbless clause \REF{ex:9:x1691}. In a few cases some other constituent is marked: a contrasted object or adverbial phrase.
...
Although the contrastive focus marking is very common when both the subject and object in an \textstyleAcronymallcaps{\textsc{osv}} clause are \textstyleAcronymallcaps{\textsc{np}}s, it is not obligatory. See \REF{ex:9:x1702} for an example.

Also in other situations where there is ambiguity about the subject, the \textstyleAcronymallcaps{\textsc{cf}} marker is used. If \REF{ex:9:x1705} did not have \textstyleAcronymallcaps{\textsc{cf}} marking, Pita would be interpreted as the possessor of the betelnuts and the meaning would be `He stole Pita's betelnuts from me'.
