...
If a [-human] patient object is more topical than a [+human] object, it can occupy the first object position. A more topical [+human] object in \REF{ex:5:} would have an unmarked third person plural pronoun before the [-human] object.\footnote{More examples of can be found in \sectref{sec:5.3.2}-5.3.4.}
...
The normal position of the peripherals is between the subject and the object \textstyleAcronymallcaps{NP,} if any \REF{ex:5:}, or between the first and second object \REF{ex:5:}. 
...
\ea%x913
\label{ex:x913}
\gll [Wiipa  nain]\textsubscript{O}1  [\textstyleEmphasizedVernacularWords{samapora} \textstyleEmphasizedVernacularWords{iinan=pa}]\textsubscript{AdvP}  [epia] \textsubscript{O}2 ururum-om-ap{\dots} \\
...
The position immediately before the verb is also the only possible place for a [+human] locative adverbial, manifested by a dative pronoun (\sectref{sec:3.5.5}). In both \REF{ex:5:} and \REF{ex:5:} there are two locative adverbials, a [-human] and a [+human] one. The [+human] locative adverbial refers to the people of the location. If it is left out, the other locative refers to the location but not the people. 

\ea%x854
\label{ex:x854}
\gll [Ni  [koka-pa]\textsubscript{AdvP}  [\textstyleEmphasizedVernacularWords{wiar}]\textsubscript{AdvP}  in-em-ik-e-man nain]\textsubscript{\textsc{rc}} kerer-omak-eka. \\
...
\ea%x855
\label{ex:x855}
\gll I  amirk=iw  [Gawar]\textsubscript{AdvP}  [\textstyleEmphasizedVernacularWords{wiar}]\textsubscript{AdvP}  urup-e-mik. \\
     1p.\textsc{unm}  day=\textsc{inst}  Gawar  3.\textsc{dat}  ascend-\textsc{pa}-1/3p \\
\glt `During the day we went to Gawar.'
\z

If there are more adverbial phrases than one, a temporal phrase normally precedes any others \REF{ex:5:}.  The relative order of the other adverbial phrases is syntactically quite free and depends on their relative topicality.
...
\ea%x930
\label{ex:x930}
\gll [\textstyleEmphasizedVernacularWords{Oposia}  \textstyleEmphasizedVernacularWords{gelemuta}]\textsubscript{O}1  [wiam  erup  fain  wia]\textsubscript{O}2 wu-om-a-m. \\
...
A left-dislocated theme (\sectref{sec:9.1}) and an afterthought are outside the clause proper.  A left-dislocated theme \REF{ex:5:} is separated from the clause by a short pause and a comma intonation, slightly rising pitch at the end of the utterance.  An afterthought \REF{ex:5:}, right-dislocated, is also separated from the rest of the clause by a short pause.
...
The verb \textstyleStyleVernacularWordsItalic{ik}- `be' seldom occurs alone \REF{ex:5:}, repeated below as \REF{ex:5:}. This is probably due to its very neutral semantic character. When it denotes being or living somewhere, it is accompanied by a locative adverbial phrase \REF{ex:5:}. Another very common adverbial phrase accompanying the verb is a manner phrase, especially the adverb \textstyleStyleVernacularWordsItalic{naap} `thus' \REF{ex:5:}. Rather than positing separate clause types with adverbials as arguments it seems reasonable to subsume clauses like these under intransitive clauses.
...
\ea%x937
\label{ex:x937}
\gll [Sawur  emeria  nain=ke]\textsubscript{S}  [\textstyleEmphasizedVernacularWords{ona} \textstyleEmphasizedVernacularWords{soma} \textstyleEmphasizedVernacularWords{mua} \textstyleEmphasizedVernacularWords{nain}]\textsubscript{O}  ifakim-o-k. \\
...
\ea%x939
\label{ex:x939}
\gll Amirika  [i]\textsubscript{S}  [\textstyleEmphasizedVernacularWords{maa} \textstyleEmphasizedVernacularWords{eneka} \textstyleEmphasizedVernacularWords{fain}]\textsubscript{O}  uup-ep enim-i-yen. \\
...
If there is only one \textstyleAcronymallcaps{NP} argument in a transitive clause, it is usually the object rather than subject \REF{ex:5:}, unless marked with the contrastive focus marker -\textstyleStyleVernacularWordsxiiptItalic{ke} \REF{ex:5:}.
...
{}
A number of ditransitive clauses \REF{ex:5:}-\REF{ex:5:} were already listed under \sectref{sec:5.1}. They belong to the three different groups below.
...
The (recipient) object is marked in the verb root of the object cross-referencing verbs denoting giving and feeding (\sectref{sec:3.8.4.2.4}), but it may appear as a separate \textstyleAcronymallcaps{NP} as well \REF{ex:5:}:

\ea%x950
\label{ex:x950}
\gll [Mua  yiar  ekap-e-mik  nain]\textsubscript{O}  [pura  kui-kuisow]\textsubscript{O} wi-e-mik. \\
...
\ea%x1839
\label{ex:x1839}
\gll I  [\textbf{opora} \textstyleEmphasizedVernacularWords{muut} \textstyleEmphasizedVernacularWords{nain}]\textsubscript{O}  [\textbf{nefa}]\textsubscript{O} \textbf{maak-u} \textbf{na-ep} ep-a-mik. \\
...
\ea%x955
\label{ex:x955}
\gll Wi  \textbf{[}\textstyleEmphasizedVernacularWords{moma}]\textsubscript{O}  \textbf{[}\textstyleEmphasizedVernacularWords{yia}]\textsubscript{O}  \textstyleEmphasizedVernacularWords{maak-i-mik},  moma=ko wi-i-yan. \\
...
\ea%x956
\label{ex:x956}
\gll [\textstyleEmphasizedVernacularWords{Waaya}]\textsubscript{O}  \textbf{[}\textstyleEmphasizedVernacularWords{yia}]\textsubscript{O}  \textstyleEmphasizedVernacularWords{na-iwkin}  waaya  wienak-em-ik-e-mik. \\
...
There are a few verbs that are ordinary transitive verbs but which can take semantically different objects. It is also possible to have one of each kind in the same clause. The verb \textstyleStyleVernacularWordsItalic{if}- `paint, spread' can have a patient or goal object; the sentence in example \REF{ex:5:} includes both. Another such verb is \textstyleStyleVernacularWordsItalic{mik}\nobreakdash- `spear, hit' see \REF{ex:5:}.  

\ea%x944
\label{ex:x944}
\gll [Yena  aasa]\textsubscript{O}  \textbf{[}ofa]\textsubscript{O}  if-e-m. \\
...
{}
When the transitivity is increased by one of the valence-increasing strategies (\sectref{sec:3.8.4.3}), a recipient or beneficiary \REF{ex:5:} becomes a second object. The linear order of the two objects depends on their relative topicality.
...
\ea%x951
\label{ex:x951}
\gll [\textstyleEmphasizedVernacularWords{Merena}]\textsubscript{O} [\textstyleEmphasizedVernacularWords{efa}]\textsubscript{O}  keraw-a-k. \\
...
\ea%x952
\label{ex:x952}
\gll [\textstyleEmphasizedVernacularWords{No} \textstyleEmphasizedVernacularWords{unuma}]\textsubscript{O}  [\textstyleEmphasizedVernacularWords{nefa}]\textsubscript{O}  faker-i-kuan. \\
...
\ea%x957
\label{ex:x957}
\gll [\textstyleEmphasizedVernacularWords{Opaimika}]\textsubscript{O}  [\textstyleEmphasizedVernacularWords{efa}]\textsubscript{O}  fien-a-man. \\
...
\ea%x948
\label{ex:x948}
\gll Era=pa  [\textstyleEmphasizedVernacularWords{ekera} \textstyleEmphasizedVernacularWords{wiam} \textstyleEmphasizedVernacularWords{erup}]\textsubscript{O1}  [\textstyleEmphasizedVernacularWords{kukusa}]\textsubscript{O2}  [\textstyleEmphasizedVernacularWords{wia}]\textsubscript{O1} aaw-o-k. \\
...
\ea%x949
\label{ex:x949}
\gll Mua  papako=ke  [\textstyleEmphasizedVernacularWords{irak-owa}]\textsubscript{O}  [\textstyleEmphasizedVernacularWords{wia}]\textsubscript{O}  puuk-a-mik. \\
...
Even three objects are allowed, but this is rare \REF{ex:5:}: the verb \textstyleStyleVernacularWordsItalic{mik}- `spear, hit' itself allows two different objects, and the third one is added via possessor raising. The objects have to be in this order. Note that in the English translation, only one direct object is allowed, and the other two phrases have to be either possessive or oblique.

\ea%x953
\label{ex:x953}
\gll [\textstyleEmphasizedVernacularWords{Keema-muuna}, \textstyleEmphasizedVernacularWords{umakuna}]\textsubscript{O}  [\textstyleEmphasizedVernacularWords{meta}]\textsubscript{O}  [\textstyleEmphasizedVernacularWords{yia}]\textsubscript{O}  \textstyleEmphasizedVernacularWords{mik-i-mik}. \\
...
The possessee is the patient-of-state subject, which is shown by the fact that it may take the contrastive focus clitic -\textstyleStyleVernacularWordsItalic{ke} \REF{ex:5:} and it determines the person inflection on the verb as well \REF{ex:5:}. The possessor is a \textstyleEmphasizedWords{\textsc{habitive adverbial}}, like  a corresponding construction in Finnish is called \citep[209]{HakulinenEtAl1979}%Karlsson
...
Because the possessee is typically inanimate and often indefinite whereas the possessor is human and definite, this causes a violation to the universal discourse-pragmatic principle, according to which animate/human and definite participants tend to precede inanimate and indefinite participants \citep[135]{Heine1997}. In order to follow the principle, Mauwake often makes the possessor a theme by moving the possessor \textstyleAcronymallcaps{NP} to sentence-initial position; only the dative pronoun keeps its position immediately preceding the verb \REF{ex:5:}. If there is no other possessor \textstyleAcronymallcaps{NP}, an unmarked pronoun is used as a theme \REF{ex:5:}. In these two sentences, moving part of the \textstyleAcronymallcaps{NP} to the theme position causes the \textstyleAcronymallcaps{NP} to be non-contiguous. In the example \REF{ex:5:} the possessee subject \textstyleStyleVernacularWordsItalic{aaya} `sugar' is also the theme, and in \REF{ex:5:} the possessee is animate/human, so in those clauses there is less pressure to make the possessor into the theme.

\ea%x973
\label{ex:x973}
\gll [\textstyleEmphasizedVernacularWords{I}]\textsubscript{T}heme  sira  naap  me  \textstyleEmphasizedVernacularWords{yiar} \textstyleEmphasizedVernacularWords{ik-ua}. \\
...
\ea%x972
\label{ex:x972}
\gll [\textstyleEmphasizedVernacularWords{Mua}  \textstyleEmphasizedVernacularWords{oko}]\textsubscript{T}heme  ona  koor  miira=pa]  [nan]  [waaya unowa]  \textstyleEmphasizedVernacularWords{wiar} \textstyleEmphasizedVernacularWords{ik-ua}. \\
...
Clauses like the example \REF{ex:5:}, where the possessed noun is [+human] and [+plural], triggering the plural form of the verb, are quite rare, and it seems that the singular verb form is also becoming possible in these cases:
...
In certain cases the verb \textstyleStyleVernacularWordsItalic{ik}- `be' is required as a copula. This happens mainly in the future \REF{ex:5:} or sometimes in the past tense, or when the clause requires a medial form to indicate that it is a medial clause \REF{ex:5:}. 
...
The classifying clauses are negated with the verbal negator \textstyleStyleVernacularWordsItalic{me} or with a clause-final negator \textstyleStyleVernacularWordsItalic{weetak/wia}.\footnote{\citet[272]{Berghall2006} also gives \textit{marew} `no(ne)' as a possible negator for equative clauses, but actually the equative clauses do not use it, only the descriptive clauses.} 
...
{}
A descriptive clause is very much like an equative clause, but the predicate is an adjective phrase \REF{ex:5:}, a noun phrase with an adjective \REF{ex:5:}, or less frequently a numeral \REF{ex:5:} or an adverbial phrase \REF{ex:5:}. On the time-stability scale these predicates are in between verbal and nominal predicates. 
...
\ea%x1226
\label{ex:x1226}
\gll Aakisa=ko  me  kerer-em-ika-i-ya,  wia  bala op-aw-ap  wia  \textstyleEmphasizedVernacularWords{wiim-om-owa}  nain.P{\footnotemark} \\
...
\z
\footnotetext{The long subject NP consisting of a nominalized clause has been right-dislocated.}

Verbal groups showing aspect may be nominalized as well, so the aspectual distinction is retained: 

\ea%x1841
\label{ex:x1841}
\gll [Mua  papako  maa  \textstyleEmphasizedVernacularWords{ik-em-ik-owa}]  nain  kawus  wiar uruf-i-kuan. \\
...
The nominalized verb in itself is neutral in regard to modality, even if it often gets deontic interpretation. But it can be, and frequently is, used in cases where modality is intentionally left unspecified. In \REF{ex:5:} the reason for not coming may be that one is not allowed, or able, or willing, to come. 
...
But note \REF{ex:5:} where the contrastive focus marker added to the nominalized verb forces a deontic interpretation. See also \sectref{sec:6.1.2}. 
...
\ea%x1235
\label{ex:x1235}
\gll [Maa  eneka  \textstyleEmphasizedVernacularWords{me} \textstyleEmphasizedVernacularWords{en-owa}]  maa  marew. \\
...
\ea%x1236
\label{ex:x1236}
\gll Wi  mua  [naap  \textstyleEmphasizedVernacularWords{me} \textstyleEmphasizedVernacularWords{on-owa}]  nain=ko  ik-e-mik=i? \\
...
Any of the four negators (\sectref{sec:6.2.1}) may be used to negate the nominalized clause \REF{ex:5:}-\REF{ex:5:}. 
...
\ea%x1237
\label{ex:x1237}
\gll [Wiena  teeria  \textstyleEmphasizedVernacularWords{baliwep} \textstyleEmphasizedVernacularWords{wia} \textstyleEmphasizedVernacularWords{kakalt-owa}]  sira  nain wia  maak-e-k. \\
...
\ea%x1842
\label{ex:x1842}
\gll [\textstyleEmphasizedVernacularWords{Ona}  \textstyleEmphasizedVernacularWords{epa} \textstyleEmphasizedVernacularWords{maneka} \textstyleEmphasizedVernacularWords{or-owa}]  \textstyleEmphasizedVernacularWords{nain}  fofa=pa  ...  unow-iya taan-e-mik. \\
...
\ea%x1843
\label{ex:x1843}
\gll [\textstyleEmphasizedVernacularWords{Niena}  \textstyleEmphasizedVernacularWords{waaya} \textstyleEmphasizedVernacularWords{mik-owa}]  \textstyleEmphasizedVernacularWords{nain}  on-ami  kuep-i-man, niena  maa=ke,  niena  wiowa=ke. \\
...
\ea%x1885
\label{ex:x1885}
\gll Dabe  wiawi  [\textstyleEmphasizedVernacularWords{maa} \textstyleEmphasizedVernacularWords{ik-ep} \textstyleEmphasizedVernacularWords{en-owa}]  na-ep manin(a)  koora  iw-a-k. \\
...
\ea%x1845
\label{ex:x1845}
\gll Toiyan  iiriw  maak-ep-pu-a-mik,  [\textstyleEmphasizedVernacularWords{uuriw} \textstyleEmphasizedVernacularWords{yia} \textstyleEmphasizedVernacularWords{aaw-ep}  \textstyleEmphasizedVernacularWords{Madang} \textstyleEmphasizedVernacularWords{ikiw-owa}]  nain. \\
...
In the following example \REF{ex:5:} the medial clause has to be within the scope of the nominalization for the sentences to make sense. The speaker had seen a possum in a tree and would have liked to shoot it, but since he had not taken his bow and arrows with him, he did not climb up the tree either.

\ea%x1844
\label{ex:x1844}
\gll [\textstyleEmphasizedVernacularWords{Nomokowa}  \textstyleEmphasizedVernacularWords{ir-ap} \textstyleEmphasizedVernacularWords{mik-owa}]  nain  yena  amia  me aaw-e-m. \\
...
The nominalized clause has several different functions. Like any other noun phrase, it may function as an argument \REF{ex:5:} or in the periphery of a clause \REF{ex:5:}, or in another noun phrase \REF{ex:5:}.

\ea%x1230
\label{ex:x1230}
\gll [\textstyleEmphasizedVernacularWords{Epia}  \textstyleEmphasizedVernacularWords{wilin-owa}]\textsubscript{O}  uruf-ap  bom  yia wafur-om-i-kuan  na-e-mik. \\
...
\ea%x1241
\label{ex:x1241}
\gll Wiena  oram  niir-emi  [\textstyleEmphasizedVernacularWords{wiam} \textstyleEmphasizedVernacularWords{kookal}\textstyleEmphasizedVernacularWords{-}\textstyleEmphasizedVernacularWords{owa=pa}]\textsubscript{Advl} nan  wiam  aaw-i-mik. \\
...
Expressions of obligation (\sectref{sec:6.1.2}) use the nominalized form of the main verb. It is followed by the contrastive focus clitic, when it is either a non-verbal predicate \REF{ex:5:} or the subject of the verb \textstyleStyleVernacularWordsItalic{ikua} `is'\REF{ex:5:}.
...
\ea%x1256
\label{ex:x1256}
\gll Iwera  popoka  wafur-am-ika-iwkin  or-op  `bulak', eewuar,  eka  \textstyleEmphasizedVernacularWords{me} \textstyleEmphasizedVernacularWords{saan-ar-owa} \textstyleEmphasizedVernacularWords{ik-ua}. \\
...
\ea%x1260
\label{ex:x1260}
\gll [\textstyleEmphasizedVernacularWords{Takira}  \textstyleEmphasizedVernacularWords{en-ow}(\textstyleEmphasizedVernacularWords{a})  \textstyleEmphasizedVernacularWords{gelemuta} \textstyleEmphasizedVernacularWords{wia} \textstyleEmphasizedVernacularWords{on-om-a-mik} 
...
\ea%x1261
\label{ex:x1261}
\gll [\textstyleEmphasizedVernacularWords{Akia}  \textstyleEmphasizedVernacularWords{ik-e-k} \textstyleEmphasizedVernacularWords{nain}]\textsubscript{\textsc{rc}}  me  en-e-k. \\
