...
It is most common to have the question in a main clause, but medial clauses also easily allow non-polar questions. The scope of the question word only extends to the clause which contains it. In \REF{ex:x1168} the fact that the people ran away is not questioned.
...
When the validity of a non-polar question \REF{ex:x1190} is questioned, the question clitic is attached directly to the end of the question already containing a question word \REF{ex:x1191}. 
...
\glt `Who did he tell???'
\z
But if the addressee wants to check if (s)he heard correctly, the echoed question is made into a complement of a sentence-final utterance verb, which gets a question clitic attached to it \REF{ex:x1192}.
...
Implied reproach or accusation is particularly common with  questions including the word \textstyleStyleVernacularWordsItalic{moram} `why?', but it is not limited to them. Especially accusations of theft are couched in neutral-looking questions \REF{ex:x1208}.
...
{}
Apart from rhetorical questions, a verbal answer is often expected. An affirmative answer to a polar question \REF{ex:x1216} may be just an affirmative interjection \REF{ex:x1217} or the verb from the question by itself or preceded by the interjection \REF{ex:x1218}. A negative answer must have at least one negator, whether only a negative interjection (\sectref{sec:6.2.3}), or any of the other negators, or both \REF{ex:x1220}. Less commonly the answer may also be a full statement with or without a preceding affirmation \REF{ex:x1219}  or negation.
...
If the speaker wants to negate the presupposition in the question, (s)he begins with a negator, and then goes on to answer the question itself \REF{ex:x1224}.
...
{}
The simple imperative is the default way of expressing a command in Mauwake. It shows in the verb inflection (\sectref{sec:3.8.3.3.2}). In a prohibition the verbal negator \textstyleStyleVernacularWordsItalic{me} `not' precedes the simple imperative \REF{ex:x1075}.
...
The imperative of the final clause may have an influence on the medial clause(s) so that they, too, are interpreted as belonging within the scope of the command. This happens very easily with same-subject medial verbs \REF{ex:x1082}; it is also possible but much less likely when the subject changes \REF{ex:x1364}\footnote{This example may also be interpreted to have two commands, a ``soft'' one, expressed with a medial verb, and a regular one.}. \REF{ex:x1846} is ambiguous: in the situation where was said, the medial clause was not in the scope of the final clause imperative; in some other situation it could be. When the medial verb has a first person form, imperative interpretation is not possible \REF{ex:x1083}. 
...
Although a command is usually directed towards one or more people in the second person, it can also be directed towards self as part of a group of two \REF{ex:x1157} or more \REF{ex:x1158}, or towards a third person in singular \REF{ex:x1159} or plural \REF{ex:x1160}. 
...
Imperatives cannot have tense distinctions, but aspectual distinctions are possible. The continuous aspect form is used for habitual in \REF{ex:x1896} and for continuous aspect in \REF{ex:x1162}. Completive aspect is used in \REF{ex:x1161} and stative in \REF{ex:x1162}.
...
The second person future tense form is also used for a command, but this is not very common. It is used in a specific situation, not for giving generic commands or rules. The sentence \REF{ex:x1080} was said to a person who was suspected of lying, and in \REF{ex:x1081} parents give instructions to their daughter how to mourn.
