...
The syntactic function provides the best criterion for defining a noun in Mauwake. Nouns function mainly as the head of a noun phrase, often the head being the only element in the \textstyleAcronymallcaps{NP}.\footnote{Sometimes an adjective, a quantifier or a genitive pronoun looks like a head of a NP, but those cases are elliptical, and the head noun is recoverable.} They can also function as a qualifier or, more rarely, as a modifier in a \textstyleAcronymallcaps{NP}. In \REF{ex:3:} \textstyleAcronymallcaps{NP}s, in this case manifested by just nouns, function as subject and object.
...
Both nouns \REF{ex:3:} and adjectives \REF{ex:3:} may be derived from verbs with the nominaliser suffix \nobreakdash-\textstyleStyleVernacularWordsItalic{owa}.
...
\textstyleEmphasizedWords{\textsc{Syntactically}} there are a few similarities between nouns and adjectives. Both can function as a modifier following the head noun in a \textstyleAcronymallcaps{NP}, although adjectives \REF{ex:3:} are much more common in this position. In {Hopper and Thompson's (1985:161)} terms, it is nouns whose categorial status has been reduced, i.e. nouns that are not fully individuated in the discourse \REF{ex:3:}, that can function in this modifier position.
...
An adjective cannot be the only element following a genitive pronoun, but a noun can. Even in elliptical expressions an adjective following a genitive pronoun is not very acceptable \REF{ex:3:}. 
...
Only an adjective functions as the head of an adjective phrase. In that position it may be modified by intensity adverbs (\sectref{sec:3.9.2}). Of these, \textstyleStyleVernacularWordsItalic{lawisiw} `rather' does not modify nouns at all \REF{ex:3:}; \textstyleStyleVernacularWordsItalic{akena} `very' and \textstyleStyleVernacularWordsItalic{pepek} `enough' may modify nouns as well; \textstyleStyleVernacularWordsItalic{wenup} `very'can do that too, but as a noun modifier it has a somewhat restricted use and a different meaning, `many'.
...
What further obscures the area of nouns and adjectives is the fact that there are a number of words that sometimes function like nouns \REF{ex:3:}, sometimes like adjectives \REF{ex:3:}, and also semantically could be like either.
...
It turns out that in Mauwake the most prototypical nouns include names of concrete \textstyleEmphasizedWords{\textsc{non}}-human rather than human objects, when one would expect words referring to human beings to be nouns \textstyleEmphasizedWords{\textsc{par excellence}} (see Taylor 1989:192). Some human nouns may be used as post-modifiers in a \textstyleAcronymallcaps{NP}: from the cluster of properties denoted by the noun one has been picked out, and the noun is used like an adjective \REF{ex:3:}, \REF{ex:3:}. The adjectival use of \textstyleStyleVernacularWordsItalic{mua} `man' in \REF{ex:3:} is particularly interesting, because the adjectives \textstyleStyleVernacularWordsItalic{morena} `male' and \textstyleStyleVernacularWordsItalic{suwina} `female' are used for animals.
...
The ``possessors'' are differentiated as first, second or third person but not as single vs. plural. An unmarked \REF{ex:3:} or a genitive \REF{ex:3:}, \REF{ex:3:} pronoun may be used to either make this number distinction or to emphasise the kin relationship, when the relationship is used as a term of reference rather than as a term of address.
...
Since there is hardly any \textstyleEmphasizedWords{\textsc{morphology}} in nouns and noun phrases, one would not expect to find much help here in distinguishing between compounds and phrases. But there is a minor factor that is relevant in this respect: a phrase containing a noun and an adjective can be pluralized by adjectival reduplication when the adjective allows reduplication \REF{ex:3:}, whereas a compound noun with a similar structure usually cannot \REF{ex:3:}, even if it is possible in some rare cases \REF{ex:3:}. 
...
Uninterruptibility is more typical of compounds than phrases. The noun phrase \textstyleStyleVernacularWordsItalic{owow maneka} means `a big village', as a compound it means `a town/city'. As a phrase it is interruptible \REF{ex:3:}, as a compound it is not.
...
The third relationship is that of \textstyleEmphasizedWords{\textsc{container}}. As a compound \textstyleStyleVernacularWordsItalic{muuk(a) sia} \REF{ex:3:} has the stress centre on the first word, in a noun phrase \REF{ex:3:} the phrase stress may also be on the second item if it is emphasized; a third person singular genitive pronoun may be added between the parts as well. Example \REF{ex:3:} is an extended compound: \textstyleStyleVernacularWordsItalic{iinan aasa} is a ``sky canoe'', or vehicle, for flying in the sky, and \textstyleStyleVernacularWordsItalic{iinan aasa epa} a place for those vehicles.
...
In the example \REF{ex:3:} the main noun \textstyleStyleVernacularWordsItalic{epa}/\textstyleStyleVernacularWordsItalic{kame} can be dropped, and this happens in some other compounds as well:
...
Action nominals function like any regular nouns in Mauwake. They can be, for example, a head \REF{ex:3:} or a qualifier \REF{ex:3:} in a \textstyleAcronymallcaps{NP}, and a first \REF{ex:3:} or last element \REF{ex:3:} in a compound noun.
...
The following expressions form an interesting pair, as \REF{ex:3:} is a \textstyleAcronymallcaps{NP} with a nominalized verb as a head, and \REF{ex:3:} is a compound noun with a nominalized verb as the first part.
...
The darkness of a colour is expressed through the adjectives \textstyleStyleVernacularWordsItalic{sepa} `black' and \textstyleStyleVernacularWordsItalic{kia} `white' used as modifiers of the main colour adjective \REF{ex:3:}.
...
Occasionally reduplication can be used for an intensifying function as well. The noun modified by the reduplicated adjective in \REF{ex:3:} is either singular or plural, in \REF{ex:3:} it is definitely singular.
...
A quantifier is the only obligatory element in a quantifier phrase (\textstyleAcronymallcaps{QP,} \sectref{sec:4.3}). These are used as post-modifiers in a \textstyleAcronymallcaps{NP}, where their position is between an adjective phrase (\textstyleAcronymallcaps{AP}) and demonstrative \REF{ex:3:}, or by themselves as a non-verbal predicate \REF{ex:3:}. 
...
Nowadays the borrowed Tok Pisin numerals have largely superseded the vernacular numerals, especially those indicating numbers ten and above \REF{ex:3:}. There are no terms for `hundred', `thousand' or bigger numbers in the vernacular system.
...
Repetition \REF{ex:3:} or reduplication \REF{ex:3:} of the numerals indicates manner: `so and so many \textstyleEmphasizedWords{\textsc{at a time}}'. The reduplicated form of \textstyleStyleVernacularWordsItalic{kuisow} `\textstyleFreeTranslationChar{one}', \textstyleStyleVernacularWordsItalic{kui-kuisow}, has two meanings: `one by one' and `a few'. 
...
A pronoun copy after a full \textstyleAcronymallcaps{NP} is hardly ever used in Mauwake for the subject. The rare example \REF{ex:3:} is from a hortatory text and may show rhetoric style:
...
The example \REF{ex:3:} is not a case of a genuine pronoun copy, since the genitive pronoun \textstyleStyleVernacularWordsItalic{wiena} adds the emphasizing meaning `themselves':
...
\subsubsection{Unmarked pronouns}
...
In an imperative clause the subject pronoun may also be used appositionally with a \textstyleAcronymallcaps{NP} that has vocative function, to address a person \REF{ex:3:}. 
...
\ea%x1776
\label{ex:x1776}
\gll \textstyleEmphasizedVernacularWords{Ni} uf-ep-na \textstyleEmphasizedVernacularWords{ni} maadara me iirar-eka ...\\
...
The third person plural unmarked pronoun is used to pluralise a noun phrase \REF{ex:3:}. It is also often used with a place name to refer to the inhabitants of the place collectively \REF{ex:3:}.
...
{}
The focal pronouns are similar to the unmarked pronouns but have final \textstyleStyleVernacularWordsItalic{-s: yos, nos, (w)os, (y)is, nis, wis.} These pronouns are never used for a neutral, non-focused subject. They are used in isolation and in lists \REF{ex:3:}, as well as with the topic marker -\textstyleStyleVernacularWordsItalic{na} \REF{ex:3:}, the contrastive focus marker -\textstyleStyleVernacularWordsItalic{ke} \REF{ex:3:}, the question marker -\textstyleStyleVernacularWordsItalic{i} \REF{ex:3:} and the adverb \textstyleStyleVernacularWordsItalic{pun} `also' \REF{ex:3:}. With the limiter -\textstyleStyleVernacularWordsItalic{iw} \REF{ex:3:} the focal pronoun forms one of the two kinds of restrictive pronoun. (See \sectref{sec:3.5.7}.)
...
The accusative pronouns are used for encoding semantic patient \REF{ex:3:}, or recipient \REF{ex:3:}, both of which are syntactic objects (\sectref{sec:5.2}, \sectref{sec:5.3}). 
...
The plural forms of the accusative pronouns are used together with the beneficiary form in the verb to disambiguate between the persons \REF{ex:3:} (\sectref{sec:3.8.3.1}).
...
\citet[52--53]{Reesink1987} mentions that Usan, another Pihom Stock language, has object prefixes, but a free pronoun can also occupy the object position in the third person singular. This is not the case in Mauwake; in \REF{ex:3:} the free pronoun \textstyleStyleVernacularWordsItalic{o} is a re-activated topic (\sectref{sec:9.2.3}). The negative clause \REF{ex:3:} shows that the position of the free pronoun is not directly preceding the verb. The clauses \REF{ex:3:}, \REF{ex:3:} have a similar structure with pronouns in non-third person marking a theme. When the pronoun is fronted as a theme (\sectref{sec:9.1}), it is this unmarked pronoun that is used in the theme position. 
...
There is one instance where the free third person singular pronoun does occur after the negator and immediately preceding the verb, just like accusative pronouns. This is when there is constituent negation (\sectref{sec:6.2.2}.) on the object, which then also receives clausal stress \REF{ex:3:} (\sectref{sec:2.1.3.1}, 9.2.3). Here it is the negator that moves to precede the constituent it negates. The same process is also seen in \REF{ex:3:} where the negator has moved in front of the whole object \textstyleAcronymallcaps{NP}. 
...
There are situations where it is impossible to determine whether the unmarked third person singular pronoun is marking a topic/subject or an object fronted as a theme (\sectref{sec:9.1}). The context would be needed to disambiguate between the slightly different meanings of \REF{ex:3:}, which do not come out well in the English translation. The first meaning is likely if the context mentions some other people seeing something; the second meaning is more probable elsewhere. 
...
There are a few verbs in Mauwake that cross-reference the patient or recipient object in the verb root (\sectref{sec:3.8.4.2.4}). These verbs do not allow a separate accusative pronoun for the function that is already expressed by the verb root \REF{ex:3:}, \REF{ex:3:}, but it is possible to have a separate accusative pronoun for the patient when the verb cross-references the recipient rather than the patient \REF{ex:3:}.
...
In descriptive or equative clauses genitive pronouns can modify both the subject \textstyleAcronymallcaps{NP} \REF{ex:3:} and the non-verbal predicate \textstyleAcronymallcaps{NP} \REF{ex:3:}, whereas the dative pronouns can modify neither. 
...
In those instances where the possessed \textstyleAcronymallcaps{NP} in the predicative position lacks an overt head noun, three different strategies may be used. I have not observed any difference in meaning. The genitive pronoun may occur by itself, without a head noun, which can either be deleted completely \REF{ex:3:} or substituted by \textstyleStyleVernacularWordsItalic{nain} `that' \REF{ex:3:}, or the \textstyleAcronymallcaps{NP} can be expressed by a possessive phrase \REF{ex:3:} (\sectref{sec:4.4}). In all these instances the head noun occurs earlier in the same sentence, or occasionally in the preceding sentence. 
...
The same dative pronoun has also grammaticalized as a possessive attribute in a noun phrase , but here it is the semantic function of [+\textstyleEmphasizedWords{\textsc{human}}] \textstyleEmphasizedWords{\textsc{source}} that is behind the development. Conceptually the structures `\textstyleAcronymallcaps{X} took \textstyleAcronymallcaps{Y} from me' and `\textstyleAcronymallcaps{X} took my \textstyleAcronymallcaps{Y}' are very close. In \REF{ex:3:} \textstyleStyleVernacularWordsItalic{efar} can mean either `my' or `from me'. 
...
In the possessor function the dative pronoun does not co-occur with the accusative pronoun in the same clause \REF{ex:3:}. In the rare occasion where there would be rivalry for the position immediately preceding the verb, the accusative is chosen \REF{ex:3:} rather than the dative \REF{ex:3:}. 
...
The use of the genitive possessive pronoun is much less restricted. Besides being employed where the possessor is coreferential with the subject \REF{ex:3:} or recipient \REF{ex:3:}, it can also be used when a possessed \textstyleAcronymallcaps{NP} is the subject or non-verbal predicate of a descriptive or equative clause \REF{ex:3:}. 
...
The genitive or unmarked pronoun may co-occur together with the dative pronoun referring to the same person, thus emphasizing the possessive function of the dative \REF{ex:3:}, \REF{ex:3:}.
...
Example \REF{ex:3:} shows how the genitive and dative possessives, in \textstyleEmphasizedWords{\textsc{different}} person forms, can modify the same noun. The dative pronoun can here be interpreted either as a possessive `your (wives)' or as a source `(wives) from you'.
...
When an isolative pronoun functions as a subject, which is \textstyleEmphasizedWords{\textsc{not}} theme (\sectref{sec:9.1}), it is alone \REF{ex:3:}; but more commonly it is both theme and subject, and is preceded by the unmarked pronoun also showing the case marking overtly \REF{ex:3:}. 
...
The example \REF{ex:3:} has an accusative pronoun to show the case and an initial unmarked pronoun \textstyleStyleVernacularWordsxiiptItalic{yo} `I' to mark the object as theme.
...
The singular forms are used as reflexives only \REF{ex:3:}, \REF{ex:3:}, the plural forms both as reflexives \REF{ex:3:}, \REF{ex:3:} and as reciprocals \REF{ex:3:}, \REF{ex:3:}.
...
In many contexts only the reflexive or the reciprocal interpretation is natural. But a potential ambiguity in some contexts is resolved by adding a genitive pronoun to mark the reflexive \REF{ex:3:} and an unmarked or restrictive pronoun to mark the reciprocal pronoun \REF{ex:3:}.
...
But a reflexive pronoun is used especially in expressions involving \textstyleParagraphChari{body parts} when one does something to oneself, and the instrument is not known or mentioned \REF{ex:3:}. In corresponding expressions English often uses possessive rather than reflexive pronouns.
...
The plural forms of the reflexive pronouns have another, quite different use: when they are followed by numerals, especially by `two' or `three', they function as dual/trial etc. forms for the personal pronouns. They are considered to be in the nominative case when not followed by other pronoun forms \REF{ex:3:}. Other cases need to be shown by appropriate additional pronouns \REF{ex:3:}.
...
The numeral modifiers are positioned between an adjective and a demonstrative in a \textstyleAcronymallcaps{NP} \REF{ex:3:}, but never between two adjectives \REF{ex:3:}.
...
Apart from the proximal demonstrative \textstyleStyleVernacularWordsItalic{fain} `this', the other demonstratives are not mutually exclusive. The distal-1 demonstrative \textstyleStyleVernacularWordsItalic{nain} `that' is the least restricted of the three, and it is extremely frequent, whereas both \textstyleStyleVernacularWordsItalic{eefin} `this/that' and \textstyleStyleVernacularWordsItalic{eenin} `that' are very rarely used. In \REF{ex:3:} the distances of the two mountains fit the specifications for \textstyleStyleVernacularWordsItalic{eefin} and \textstyleStyleVernacularWordsItalic{eenin} , and more than one distal demonstrative is needed for contrastive purposes:
...
With [-human] nouns, a quantifier in the \textstyleAcronymallcaps{NP} may be used \REF{ex:3:}, or distributive suffix on the verb \REF{ex:3:} to indicate plurality, or the number may be left unspecified \REF{ex:3:}.
...
Mauwake follows the typical pattern: the neutral distal demonstrative \textstyleStyleVernacularWordsItalic{nain} `that' is anaphoric: it only refers to the text preceding it, as in \REF{ex:3:}, where the example sentence comes after the description of fishing with a fish trap. The proximal \textstyleStyleVernacularWordsItalic{fain} `this'is cataphoric, referring to the text following it \REF{ex:3:}. The other two demonstratives, \textstyleStyleVernacularWordsItalic{eefin} and \textstyleStyleVernacularWordsItalic{eenin}, are not used for text-internal reference at all.
...
Still another function for the demonstrative \textstyleStyleVernacularWordsItalic{nain} `that' is that of a nominaliser of otherwise finite verbal clauses (\sectref{sec:5.7.2}). A nominalized clause of this type may be a relative clause \REF{ex:3:} (\sectref{sec:8.3.1}), a complement clause \REF{ex:3:} (\sectref{sec:8.3.2}) or a temporal subordinate clause \REF{ex:3:} (\sectref{sec:8.3.3.1}).\footnote{All these clauses have a function that is consistent with the core meaning of `givenness' \citep{Haiman1978} or presupposition \citep{Reesink1987}.}
...
The same demonstrative is also used as a strong adversative `but' \REF{ex:3:} (\sectref{sec:8.1.3}). In that function it is placed clause-initially rather than clause-finally.
...
In \REF{ex:3:} there is a long temporal distance between the hearing and the recounting of the story, which is apparently reflected in the choice of the adverbial.
...
In \REF{ex:3:} there is both some temporal and a considerable locative distance between the original time and place of the quote and that of the rest of the example: 
...
\textstyleStyleVernacularWordsItalic{Naarew}\textstyleStyleVernacularWordsItalic{(e)} `who?' is only used for [+human] referents. When the contrastive focus maker -\textstyleStyleVernacularWordsItalic{ke} is suffixed to the question word, the last syllable is normally deleted. \textstyleStyleVernacularWordsItalic{Mauwa} `what', on the other hand, is used almost solely for [-human] nouns. The only natural expression with \textstyleStyleVernacularWordsItalic{mauwa} referring to humans that I have encountered is of the type \REF{ex:3:}. When a person's name is inquired, either \textstyleStyleVernacularWordsItalic{naarewe} \REF{ex:3:} or \textstyleStyleVernacularWordsItalic{kamenap} \REF{ex:3:} is used rather than \textstyleStyleVernacularWordsItalic{ma}\textstyleStyleVernacularWordsItalic{uwa}.
...
\textstyleStyleVernacularWordsItalic{Kaanin} `which of two?' is specified for number \REF{ex:3:}, but \textstyleStyleVernacularWordsItalic{kain} `which?' is not.
...
\textstyleStyleVernacularWordsItalic{Kamenap} is a question word both for manner `how?' \REF{ex:3:} and for adjectives `what {\dots} like?'. In the latter sense it usually modifies the noun \textstyleStyleVernacularWordsItalic{sira} `custom, kind' \REF{ex:3:}.
...
The verbal predicate is the only obligatory element in an intransitive clause. A transitive clause does require an object, but even it can often consist of a verb only, as the third person singular accusative pronoun, used for object, is zero \REF{ex:3:}. The directional verbs (\sectref{sec:3.8.4.4.5}) often co-occur with a goal, but when it is left implied the verb can be the only element \REF{ex:3:}. In a verbless clause the predicate is a noun, adjective, possessive pronoun or adverb. 
...
\subsubsection{Category-changing derivation: verb formation}
...
Semantically the resulting verb is usually very close to the word that serves as the root, but in a few instances like \REF{ex:3:} the semantic link is not very strong.
...
The full resultative verb \textstyleStyleVernacularWordsItalic{ar}\textstyleEmphasizedVernacularWords{-} `become' is more common with nouns, and the meaning of the verb is transparent \REF{ex:3:}. It is also used with numerals \REF{ex:3:}. Both words retain their word stress.
...
The inchoative suffix \textstyleEmphasizedVernacularWords{\nobreakdash-}\textstyleStyleVernacularWordsItalic{ar} can occur with nouns \REF{ex:3:}, but is more common with adjectives \REF{ex:3:} and adverbs \REF{ex:3:}, and can attach to a few verb roots too \REF{ex:3:}. Since the result is one word it only has one word stress. 
...
Verbs derived from adjectives are often used rather than adjectives in the predicative position \REF{ex:3:}, \REF{ex:3:}. And instead of a modifying adjective, a whole relative clause with a verb derived from an adjective may be used \REF{ex:3:}. This happens especially when the property denoted by the adjective is not static. 
...
The suffix \textstyleStyleVernacularWordsItalic{ar}- often retains its original verbal meaning `become' when adjectives are made into verbs \REF{ex:3:}, but when the other word classes are used as the root the original meaning tends to become more opaque or get lost \REF{ex:3:}. 
...
The hierarchy of which argument the distributive applies to is as follows: if there is a recipient \REF{ex:3:} or beneficiary \REF{ex:3:}, the distributive applies to that; if there is no recipient or beneficiary but an object, the distributive applies to the object \REF{ex:3:}; and in case the clause has neither a recipient or beneficiary nor an object, the distributive applies to the subject \REF{ex:3:}. Since transitive verbs need an object, the subject can be pluralized with the distributive only when the verb is intransitive.
...
In verbal groups the benefactive suffix is usually attached to the finite verb or auxiliary \REF{ex:3:} but can occasionally occur on the non-finite root \REF{ex:3:} or even on both of the two \REF{ex:3:}.
...
The benefactive form does not always mean that something happens for someone's\textstyleEmphasizedWords{} \textstyleEmphasizedWords{\textsc{benefit}}. The benefactive may be strengthened with the adverb \textstyleStyleVernacularWordsItalic{orawin} `for the benefit' \REF{ex:3:}, which makes it unambiguous.
...
In verbs of motion reduplication means continuity \REF{ex:3:}, and the passing of time may be shown by the number of reduplications \REF{ex:3:}.
...
In other intransitive verbs reduplication indicates either iterative action \REF{ex:3:} or occasionally continuity \REF{ex:3:}.
...
Both of these meanings fit in well with \citegen[319]{Moravcsik1978}'s description of the various meanings that reduplication in verbs can have. In transitive verbs reduplication indicates iterative action as well as the plurality of an inanimate object \REF{ex:3:}. The form is used especially when the action \textstyleEmphasizedWords{results} in a plural object \REF{ex:3:}.
...
\paragraph[Bring-\textsc{pr}efixes]{Bring-\textsc{pr}efixes}
...
The only two forms for the beneficiary are \textstyleEmphasizedVernacularWords{\nobreakdash-}\textstyleStyleVernacularWordsItalic{e} for first or second person singular \REF{ex:3:} and \textstyleEmphasizedVernacularWords{\nobreakdash-}\textstyleStyleVernacularWordsItalic{a} for all the other persons \REF{ex:3:}. The context often provides more person distinctions, as the plural requires accusative pronouns to precede the verb to indicate the beneficiary, like third person plural in \REF{ex:3:}. 
...
If there is a beneficiary suffix \textstyleStyleVernacularWordsItalic{-a} preceding the counterfactual, the vowel /e/ of the counterfactual suffix is deleted \REF{ex:3:}:
...
The initial (or only) vowel in the second person imperative forms is deleted after the beneficiary suffix \REF{ex:3:}-\REF{ex:3:}. 
...
\subsubsection{Tense and person/number in final verbs}
...
Besides their primary meaning, the present and future tenses also have secondary meanings. The present tense form of the first or third person plural is used for generic or time-neutral statements \REF{ex:3:}. For the habitual aspect in the present, the simple present tense \REF{ex:3:} is an alternative to the full habitual aspect form.\footnote{Continuous aspect form is required for the past habitual (\sectref{sec:3.8.5.1.1.2}).} 
...
\subsubsection{Medial verb marking}
...
If the verb has the beneficiary suffix \textstyleStyleVernacularWordsItalic{-a} or \textstyleStyleVernacularWordsItalic{-e} (\sectref{sec:3.8.3.1}), the vowel of the medial verb suffix gets assimilated to it \REF{ex:3:}, \REF{ex:3:}.
...
The verb \textstyleStyleVernacularWordsItalic{ik}- `be' is different from other verbs in that there is no differentiation between the simultaneous and sequential forms in the same-subject medial verb: in example \REF{ex:3:} the actions are simultaneous, in \REF{ex:3:} sequential. Also, the verb does not take either one of the normal same-subject suffixes.
...
The suffix is -\textstyleStyleVernacularWordsItalic{aya} instead of \nobreakdash-\textstyleStyleVernacularWordsItalic{eya} in a few short conjugation class 1 verbs (\sectref{sec:3.8.4.1}) \REF{ex:3:}\footnote{The vowel \textstyleFootnoteBaseChar{\textit{-a}} is somewhat more common in the Muaka dialect group where the 2/3s.\textsc{ds} marker is -\textit{era} instead of \nobreakdash-\textit{eya.}} and in those benefactive verbs where the first vowel of the suffix is assimilated to the preceding vowel of the beneficiary suffix \REF{ex:3:}. 
...
{}
The medial verbs have no tense marking, so the tense in a medial clause is interpreted in relation to that of the next finite clause. When the finite clause is in the past tense, both a simultaneous \REF{ex:3:} and a sequential medial clause \REF{ex:3:} are also understood to be in the past tense.
...
The medial verbs acquire more absolute-relative tense character of ``past in the past'' \citep[65]{Comrie1985} in those cases where sequential medial clauses are either right-dislocated and placed after the final clause \REF{ex:3:} or placed inside another medial clause \REF{ex:3:}, or when there is a separate time expression referring to earlier time than that indicated by the final verb \REF{ex:3:}. 
...
\subsubsection{Verb classes based on transitivity}
%\hypertarget{RefHeading20561935131865}
{}
With the term \textstyleEmphasizedWords{\textsc{transitivity}} of a verb I refer to its \textstyleEmphasizedWords{\textsc{syntactic}} transitivity, i.e. ``{the number of overt morpho-syntactically coded arguments it takes}'' (Van Valin and LaPolla 1997:147). 
...
{}
In Mauwake the class of basic, or ``ordinary'', intransitive verbs consists of a semantically very diverse group including involuntary processes \REF{ex:3:}, many motion verbs \REF{ex:3:}, and some bodily function verbs \REF{ex:3:}.
...
Besides the prototypical transitive verbs with an agent subject and a patient-of-change object (Giv\'on 1984:96) like \REF{ex:3:} and \REF{ex:3:}, also many verbs of perception \REF{ex:3:}, cognition \REF{ex:3:} and emotion \REF{ex:3:} are transitive.
...
Also, if the directional verbs were considered weakly transitive and the the goal a locative object, the locative adverb \textstyleStyleVernacularWordsxiiptItalic{nan} `there' in the following clauses would be treated as a locative adverb in \REF{ex:3:} but as a locative object in \REF{ex:3:}:
...
Unlike Usan, in Mauwake the clauses with object cross-referencing verbs can easily have a locative or instrument phrase, and the verb itself can take a benefactive suffix. A sentence like \REF{ex:3:} would be possible for instance when sending money to people travelling in the same vehicle as the addressee. 
...
The mental state of being angry is expressed via a verb in Mauwake \REF{ex:3:}, and it can take the causative suffix \REF{ex:3:}.
...
{}
The benefactive form of a verb (\sectref{sec:3.8.2.3.3}) is used when an action is done \textstyleEmphasizedWords{\textsc{for} }someone, for their benefit \REF{ex:3:}, or in some cases for their detriment \REF{ex:3:}. With the addition of the benefactive suffix to the verb, the beneficiary becomes an obligatory argument. The beneficiary is always animate, and usually human. 
...
More than one valency-increasing strategy can be applied to a verb simultaneously. In both \REF{ex:3:} and \REF{ex:3:} the valency of the verb increases from one to three: besides the subject of the original verb, the derived verbs also have both an object and a beneficiary.
...
\subsubsection{Semantically based verb classes} 
...
Like intransitive verbs, it may form a complete clause by itself. Example \REF{ex:3:} is from a situation where the speaker was in a plane for the first time, refused to eat and declined any help offered to him.
...
Together with the dative pronouns \textstyleStyleVernacularWordsItalic{ik}- is used to form possessive constructions \REF{ex:3:} (\sectref{sec:3.5.5}, 5.5.2). 
...
In Mauwake it can also be used when the non-verbal predicate is understood to be transitory \REF{ex:3:} rather than stable over time:
...
An equative or descriptive medial clause requires \textstyleStyleVernacularWordsItalic{ik}- as a copula regardless of the tense of the final verb \REF{ex:3:}.
...
{}
The three position-taking verbs are among the most frequently used verbs in Mauwake: \textstyleStyleVernacularWordsItalic{pok}- `sit down', \textstyleStyleVernacularWordsItalic{iimar}- `stand up' and \textstyleStyleVernacularWordsItalic{in}- `lie down/ fall asleep'. They are essentially punctiliar verbs with an inceptive meaning \REF{ex:3:}, but they are most typically used in the same-subject sequential form together with the auxiliary \textstyleStyleVernacularWordsItalic{ik}- (\sectref{sec:3.8.4.5}) to convey stative meaning: `sit' \REF{ex:3:}, `stand', and `lie/sleep'.
...
The deictic orientation of \textstyleStyleVernacularWordsItalic{ikiw}- `away from speaker/deictic centre' and \textstyleStyleVernacularWordsItalic{ekap}- `towards the speaker/deictic centre'is stricter in Mauwake than in many European languages where the deictic centre especially for `come' can vary considerably. The sentence \REF{ex:3:} is all right in Finnish regardless of the location of the speaker, but the corresponding sentence in Mauwake would be acceptable only if the speaker were in Tampere at the time of speaking.
...
The equivalent of the English `come' in \REF{ex:3:} has to be `go' in Mauwake \REF{ex:3:}. This is discussed further in 6.3. 
...
The directional verbs are very frequent as the second root in serial verbs \REF{ex:3:} (\sectref{sec:3.8.5.1.2}) and as the main verb in verb plus auxiliary constructions \REF{ex:3:} (\sectref{sec:3.8.5.1.1}). Some of them also enter into adjunct plus verb constructions \REF{ex:3:} (\sectref{sec:3.8.5.2}). 
...
The meaning of the verbs \textstyleStyleVernacularWordsItalic{ekap}- `come' and \textstyleStyleVernacularWordsItalic{ikiw}- `go' can be metaphorically extended to time, to signal time spans. The former is used when the time span is extended from the past to the present \REF{ex:3:}, the latter is more common when the time extends from the present to the future \REF{ex:3:}, but it can also refer to the past \REF{ex:3:}.
...
With the other two a bring-\textsc{pr}efix is acceptable \REF{ex:3:}, but they do not take a goal/path argument. If a locative phrase occurs with them it requires a locative clitic \REF{ex:3:}.
...
The verb \textstyleStyleVernacularWordsItalic{maak}- `tell' is used in the same two main senses as its English equivalent: telling someone \textstyleEmphasizedWords{\textsc{about}} something \REF{ex:3:} and telling someone \textstyleEmphasizedWords{\textsc{to do}} something \REF{ex:3:}. In direct quote formulas it is used mainly preceding a quote \REF{ex:3:}, not directly following it as a short closing formula. It is not used in indirect quotes at all.
...
The status of the verb \textstyleStyleVernacularWordsItalic{naak}- is unclear. It is infrequent, and in natural texts only occurs in closing formulas \REF{ex:3:}. It may have developed as an analogy to the verb pair \textstyleStyleVernacularWordsItalic{ma}-/\textstyleStyleVernacularWordsItalic{na}-.
...
With the verb \textstyleStyleVernacularWordsItalic{ma}- `say/speak/tell' the addressee is not in focus, and is hardly ever even mentioned. Instead, the verb requires either an object referring to the speech content \REF{ex:3:} or an adverb \textstyleStyleVernacularWordsItalic{naap} `thus' \REF{ex:3:} preceding the verb, or a quote complement following it \REF{ex:3:}. 
...
The verb \textstyleStyleVernacularWordsItalic{na}- `say/speak/call/think' is the most interesting of the speech verbs. In quote formulas it is only used for closing the quote \REF{ex:3:}, with or without another utterance verb in an opening formula.
...
When the main verb is in the same-subject sequential form (\textstyleAcronymallcaps{\textsc{ss}.\textsc{seq}}), the auxiliary \textstyleStyleVernacularWordsItalic{ik}- indicates stativity \REF{ex:3:}. With non-punctiliar verbs this form can often be translated into English with a past perfect \REF{ex:3:}.
...
\z
 ...
 When the verbal group is in the medial form, the progressive interpretation \REF{ex:3:} is the more common:
...
Often the context provides the only clue as to whether the continuous aspect form should be interpreted as progressive or habitual. The example \REF{ex:3:} describes a situation where the villagers kept feeding the Japanese soldiers who asked them for food; the sentence \REF{ex:3:} is from a text describing a coconut plantation fire and its consequences.
...
Cross-linguistically the habitual aspect more commonly receives overt marking in the past tense than in the present \citep[154]{Cristofaro2006}. In Mauwake the continuous aspect can be used for habitual in any of the three tenses. The example \REF{ex:3:} was said about particular work that the speaker was not involved in continuously; he used to do it time to time because of his position as need arose. The example \REF{ex:3:} refers to a couple needing to keep visiting an ailing father. 
...
Other punctiliar verbs \REF{ex:3:}, as well as non-punctiliar verbs can be used in this aspect to indicate the state resulting from an action \REF{ex:3:}, or process \REF{ex:3:}, but they are less frequent.
...
A motion verb in a serial construction can also indicate \textstyleEmphasizedWords{\textsc{temporal continuity}} over a long period of time. In \REF{ex:3:} the length of time is emphasized even more by the repetition of the motion verb.
...
In Mauwake the clause chaining is structurally midway between serialization and main clause coordination, and may consequently be used instead of either in some cases. The instrumental may in Mauwake be expressed by a `take-instrument-do' structure \REF{ex:3:} which in many serializing languages is a serial verb construction \citep[162--74]{Sebba1987}; but in Mauwake there is no good reason to call the structure anything other than a combination of a medial and final clause. This shows more clearly in example \REF{ex:3:}, which does not pass the rule for verbal groups: ``no non-verbal elements between the parts''. 
...
Likewise, the four verbs in \REF{ex:3:} describe \textstyleEmphasizedWords{\textsc{one}} protracted action in spite of the sequential form in the medial verbs:
...
\subsubsection{Adjunct plus verb constructions} 
...
Also, if the action is the same but the situation changes, \textstyleStyleVernacularWordsItalic{kiiriw} is used. The example \REF{ex:3:} describes a situation where a grandmother first sent her younger grandchild, a girl, to listen to a sound. Later she sent the grandson for the same errand; the act of sending was repeated but the person who was sent changed:
...
\textstyleStyleVernacularWordsItalic{Eliw} `all right, well'\footnote{The manner adverb `well' is \textstyleFootnoteBaseChar{\textit{baliwep} }\textstyleFootnoteBaseChar{(\sectref{sec:3.8.1.3})}.} is a deontic adverb and expresses permission or desirability: `it is all right/good that{\dots}'. It can often be translated with the auxiliary `may' in English. It follows the subject, if there is any, but precedes the other clause constituents \REF{ex:3:}. It may also be in the tail position after the clause, either following a clause that already has \textstyleStyleVernacularWordsItalic{eliw} in it \REF{ex:3:}, or by itself \REF{ex:3:}. 
...
Words or phrases in lists are most commonly joined by juxtaposition only. If a connective is used, \textstyleStyleVernacularWordsItalic{ne} usually joins the last two \REF{ex:3:} coordinands. It is also possible to place the connective(s) closer to the beginning of the list.
...
\textstyleStyleVernacularWordsItalic{Aria} `alright'\footnote{The translation reflects the Tok Pisin word \textit{orait}, which sometimes has a similar discourse function. \textit{Aria} occurs in many Madang languages, and the speakers of those languages tend to use \textit{aria} in Tok Pisin too.} usually comes sentence-initially, but can also or occur sentence-medially. Its main function is to indicate a break in the topic chain. In \REF{ex:3:} the topic changes from the snake to the man, and in \REF{ex:3:} from a health extension officer to a group of men:
...
It often signals the beginning of a turn in a conversation \REF{ex:3:}, or beginning of a speech \REF{ex:3:}, again indicating a break with the preceding text. 
...
Even if the topic stays the same, \textstyleStyleVernacularWordsItalic{aria} can be used, especially when there is a contrast between alternatives \REF{ex:3:}, or sometimes when an expected sequence of events is broken \REF{ex:3:}.
...
When there are more alternatives than one and the question clitic is present, the connective may be left out altogether \REF{ex:3:}, or it may occur between the first two alternatives \REF{ex:3:}. 
...
The origin of \textstyleStyleVernacularWordsItalic{naeya} is so transparent that there are many cases where two different interpretations for \textstyleStyleVernacularWordsItalic{naeya} are acceptable \REF{ex:3:}. 
...
The originally dialectal difference may be developing into a semantic one. In the original text data from three decades ago there is no clear semantic distinction between the use of \textstyleStyleVernacularWordsItalic{naapeya} and \textstyleStyleVernacularWordsItalic{naeya}, but fairly recently when a group with members from different dialects, discussing language matters, produced consecutive clauses, nearly all of the sentences with \textstyleStyleVernacularWordsItalic{naapeya} were cases of cause-effect \REF{ex:3:}, and all of the sentences with \textstyleStyleVernacularWordsItalic{naeya} were cases of reason and result \REF{ex:3:}. 
...
The dual comitative \textstyleStyleVernacularWordsItalic{ikos} `with, together (with)' can only be used when two human participants are referred to \REF{ex:3:}. It can also occur alone, without a preceding noun phrase, when the participants are known from the person/number suffix in the verb, or from the context \REF{ex:3:}. The parties are considered equally active, so the verb is always in the plural.
...
{}
\textstyleStyleVernacularWordsItalic{Muuta}\textstyleStyleVernacularWordsItalic{ (nain)} `because of, for' gives a reason for an action, when the reason is expressed in a noun phrase rather than a full clause. It has developed from the adverb \textstyleStyleVernacularWordsItalic{muuta} `a little, only', and in some cases the meaning `only' is retained with the new function as well \REF{ex:3:}, \REF{ex:3:}. The distal-1 demonstrative \textstyleStyleVernacularWordsItalic{nain} `that' is optional, and is left out especially when there is another demonstrative \textstyleStyleVernacularWordsItalic{nain} preceding \textstyleStyleVernacularWordsItalic{muuta} \REF{ex:3:}.
...
{}
\textstyleStyleVernacularWordsItalic{Saarik} `like' occurs with noun phrases \REF{ex:3:} and with nominalized clauses \REF{ex:3:}. It indicates a point of similarity between two essentially \textstyleEmphasizedWords{different} things.
...
{}
The locative clitic -\textstyleStyleVernacularWordsItalic{pa} mainly marks locative in noun phrases \REF{ex:3:}. The most common verb that it collocates with is \textstyleStyleVernacularWordsItalic{ik}- `be' \REF{ex:3:}. When it occurs with the directional verbs (\sectref{sec:3.8.4.4.5}), it often indicates source \REF{ex:3:}, but it can also be used for path \REF{ex:3:}, or for instrument in cases where it has a strong locative meaning as well \REF{ex:3:}. It is rarely used for goal \REF{ex:3:}; this is possible in cases where the goal is the location for an event taking place immediately.
...
As temporal phrases locate an event in time, they also use the same locative clitic \REF{ex:3:}.
...
The locative clitic has its origin in the word \textstyleStyleVernacularWordsItalic{epa} `place'; the transition vowel [e] can sometimes be heard between the clitic and its host, when the host word ends in a consonant \REF{ex:3:}.
...
If the head noun of the \textstyleAcronymallcaps{NP} is is recoverable from the context, it may be deleted, leaving behind only \textstyleStyleVernacularWordsItalic{ena}. In \REF{ex:3:} the head noun \textstyleStyleVernacularWordsItalic{epira} `bowl(s)' has been omitted.
...
Example \REF{ex:3:} is from a traditional story, where a man has gone hunting and the spirit of his lover comes to his home. When the wife sees her, she knows what the spirit woman has come to look for and comments:
...
In \REF{ex:3:} the answer to the question reveals the identity of the person asked about; the topic marker may be used even in a short exchange like this but especially if the text continues to tell more about the topic. 
...
In \REF{ex:3:} the speaker changes the topic to the addressee after a discussion on something else:
...
It is also used in adversative subordinate clauses (\sectref{sec:8.3.4}) when the main clause expresses a frustrated effort or a cancelled expectation \REF{ex:3:}, or surprise.
