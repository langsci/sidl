...
When two main clauses are joined in a coordinate sentence, they are independent of each other as to their functional sentence type. In \REF{ex:8:x1352} the first clause is declarative and the second one interrogative; in \REF{ex:8:x1358} the first clause is imperative and the second one declarative, but the order could also be reverse. 
...
The following example \REF{ex:8:x1373} is syntactically neutral, but semantically it is interpreted as both temporal and consecutive sequence.
...
\textstyleStyleVernacularWordsItalic{Neemi}  is a consecutive coordinator that almost exclusively conjoins full sentences rather than clauses within a sentence: \REF{ex:8:x1904} is from translated text but considered natural. \REF{ex:8:} is repeated here as \REF{ex:8:x1409}. \textit{Neemi} is an internal connective, only used in reasoning. It requires some point of similarity between the two conjuncts.
...
Clause chaining in Mauwake distinguishes between sequential and simultaneous actions in the clauses joined by chaining, but only when the clauses have the same subject (\sectref{sec:3.8.3.5.1}). The sequential action verb \REF{ex:8:x1431} indicates that one action is finished before the next one starts. 
...
Adding new derivation to the medial verb is possible, but rare: the example \REF{ex:8:} is repeated below as \REF{ex:8:x1515}.
...
\z
 ...
 Similarly, the aspect marking normally stays the same in both the verbs, but it is also possible to have aspect marking on the medial verb, although the finite verb is without any aspect marking \REF{ex:8:x1516}. When new information is added to the verb either by derivation or aspect marking, it is less clear if this still is a true case of tail-head linkage.  
...
The tense in the \textstyleAcronymallcaps{\textsc{rc}} can be past \REF{ex:8:} or present \REF{ex:8:}, but not future. For future meaning, the present tense form has to be used \REF{ex:8:x1531}. 
...
As was mentioned above, the \textstyleAcronymallcaps{\textsc{AntNP}} only rarely shows overtly. But a \textstyleAcronymallcaps{\textsc{RelNP}} can also be deleted if it is generic \REF{ex:8:}, or recoverable from situational \REF{ex:8:} or textual context \REF{ex:8:}. In the example \REF{ex:8:x1555} the deleted \textstyleAcronymallcaps{\textsc{RelNP}} can either be generic `what/whatever' or it may be \textstyleStyleVernacularWordsItalic{opora} `talk'; in \REF{ex:8:x1559} the speaker is describing the process of making a fishtrap, which has already been mentioned in previous sentences.
...
In \REF{ex:8:x1556} there is no other indication of the  \textstyleAcronymallcaps{\textsc{RelNP}} than the person suffix of the verb. The group of women referred to were mentioned as a noun phrase only near the beginning of the story, whereas the example is from close to the end:
...
Recipient \REF{ex:8:x1539} and beneficiary \REF{ex:8:x1547} are possible to relativise, but in natural texts especially beneficiary is very infrequent.
...
The object of genitive, or object of \textstyleEmphasizedWords{\textsc{possessive}} as it should be called when describing Mauwake grammar, only uses the dative pronoun (\sectref{sec:3.5.5}) when relativized \REF{ex:8:x1543}, not the unmarked (\sectref{sec:3.5.2.1}) or genitive (\sectref{sec:3.5.4}) pronoun.
...
The verb \textstyleStyleVernacularWordsItalic{ma}- `say, talk' can take a regular complement clause, which is of the fact type \citep[389]{Dixon2010b}. This clause functions as an object of the verb in the same way as a \textstyleAcronymallcaps{\textsc{np}} with the head noun \textstyleStyleVernacularWordsItalic{opora} (or \textstyleStyleVernacularWordsItalic{opaimika}) `talk/story' in \REF{ex:8:x1595}:
...
Both types of a nominalized clause (\sectref{sec:5.7.1}, 5.7.2) may be used as subjects in verbless clauses, even though this function for complement clauses is not common. A clause with a nominalized verb is used when the activity is potential \REF{ex:8:x1636}, \REF{ex:8:x1637}. 
...
Also those instances where the conditional marker is attached to a predicate that is not originally a verb, the predicate needs to have medial verb marking (\sectref{sec:3.8.3.4.2}\todo{please check}).
...
The chaining structure is also used to coordinate relative clauses \REF{ex:8:x1463} and complement clauses that have a nominalized verb \REF{ex:8:}, copied as \REF{ex:8:x1848} below: 
