...
A simple sentence is the same as a clause, and was discussed in \chapref{chap:5}.
...
When the final clause is in the imperative mood, the implication of a command often extends backwards to a medial verb marked for the same subject \REF{ex:8:x1365}, but not so easily to one marked for a different subject. In \REF{ex:7:x1364} above the command /request extends to the medial clause, whereas in \REF{ex:8:x1356} it does not. For more examples, see \REF{ex:7:x1082}-\REF{ex:7:x1083} above.
...
For more examples, see \REF{ex:7:x1082}-\REF{ex:7:x1083} in \sectref{sec:7.3} and the introductory section to \chapref{chap:8}. 
...
Like coordinated main clauses and unlike subordinate clauses, medial clauses are not embedded as constituents in other clauses. However, a medial clause may interrupt its reference clause and appear inside it, if the subject or object \textstyleAcronymallcaps{NP} of the reference clause is fronted as the theme and thus precedes the interrupting medial clause. For more examples, see \REF{ex:3:x539} and \REF{ex:3:x540}. In the following examples the reference clause is bolded and the intervening medial clause is placed within square brackets.
...
Adding new derivation to the medial verb is possible, but rare: the example \REF{ex:3:x237} is repeated below as \REF{ex:8:x1515}.
...
As was mentioned above, the \textstyleAcronymallcaps{\textsc{AntNP}} only rarely shows overtly. But a \textstyleAcronymallcaps{\textsc{RelNP}} can also be deleted if it is generic \REF{ex:8:x1561}, or recoverable from situational \REF{ex:8:x1555} or textual context \REF{ex:8:x1559}. In the example \REF{ex:8:x1555} the deleted \textstyleAcronymallcaps{\textsc{RelNP}} can either be generic `what/whatever' or it may be \textstyleStyleVernacularWordsItalic{opora} `talk'; in \REF{ex:8:x1559} the speaker is describing the process of making a fishtrap, which has already been mentioned in previous sentences.
...
Below in \REF{ex:8:x1266} also the person suffix is changed to reflect the situation of the new speech act participants. 
...
Polar questions, when indirect, have to be alternative questions. The verb \textstyleStyleVernacularWordsItalic{naep} may be deleted, when the indirect question is sentence-final \REF{ex:8:x1593}. 
...
Purpose is both conceptually close and structurally similar to desiderative, and in Mauwake many of the desiderative clauses can be interpreted as purpose clauses. This is particularly so when the main verb is in the nominalized form. But a truly desiderative clause even with an action nominal is never right-dislocated, whereas a purpose clause \REF{ex:8:x372} often is. The nominalized form in the main verb is common:
...
The chaining structure is also used to coordinate relative clauses \REF{ex:8:x1463} and complement clauses that have a nominalized verb \REF{ex:5:x1845}, copied as \REF{ex:8:x1848} below: 
