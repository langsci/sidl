...
Theoretically it is quite possible, and grammatically correct, to have a \textstyleAcronymallcaps{NP} like the one in \REF{ex:4:}, but natural language data seldom has any \textstyleAcronymallcaps{NP}s with more than two modifiers; example \REF{ex:4:} is one of those.
...
The only modifier in a noun phrase most typically is either a possessor \REF{ex:4:}, a deictic \REF{ex:4:} or a qualifying noun or noun phrase \textstyleAcronymallcaps{NP}. In \REF{ex:4:} the qualifying noun is a compound noun.
...
An \textstyleEmphasizedWords{\textsc{unmarked third person plural pronoun}} is used as an optional plural marking for humans and other human-like beings \REF{ex:4:}, \REF{ex:4:}. 
...
The plural-marking pronoun differs from the appositive use \REF{ex:4:} of the unmarked pronoun in that the former is unstressed, whereas the latter is stressed and, furthermore, may be any person and either singular or plural. (For appositional NPs, see \sectref{sec:4.1.4}.)
...
A special case of the plural-marking unmarked pronoun is where it occurs with a place name to refer to the people of that place \REF{ex:4:}.  The head noun \textstyleStyleVernacularWordsItalic{mua} `men, people' or \textstyleStyleVernacularWordsItalic{emeria} \textstyleStyleVernacularWordsItalic{mua} `people' is not needed; it may be used, but is usually left out.
...
A \textstyleEmphasizedWords{\textsc{temporal phrase}} is rare as a \textstyleAcronymallcaps{NP} constituent. Mainly the temporal words \textstyleStyleVernacularWordsItalic{aakis}  'present-day' from \textstyleStyleVernacularWordsItalic{aakisa} 'now, today' \REF{ex:4:} and \textstyleStyleVernacularWordsItalic{iiriw} 'earlier' \REF{ex:4:} may be used, but a temporal phrase is also allowed : 
...
The head noun of a \textstyleEmphasizedWords{\textsc{possessive} }\textstyleAcronymallcaps{\textup{NP}} can only be [+human], with `human' including spirits \REF{ex:4:} and sometimes some domestic animals like dogs or pigs \REF{ex:4:}. The humanness of the \textstyleAcronymallcaps{PossNP} is stressed by the fact that it may be followed by a pronoun copy in the genitive \REF{ex:4:}.
...
\ea%x405
\label{ex:x405}
\gll \textstyleEmphasizedVernacularWords{i}  \textstyleEmphasizedVernacularWords{emeria} \textstyleEmphasizedVernacularWords{apura} \textstyleEmphasizedVernacularWords{yiena} \textstyleEmphasizedVernacularWords{mua} \textstyleEmphasizedVernacularWords{weria}  emeria  nain=ke \\
...
The semantic relation of the ``possessor'' to the ``possessed'' may be that of real ownership, paraphrasable with `have' \REF{ex:4:}, a human relationship \REF{ex:4:}, origin \REF{ex:4:} or subjecthood \REF{ex:4:}. 
...
An unmarked pronoun used possessively is often stressed in speech \REF{ex:4:}.
...
In recursive genitive structures like \REF{ex:4:} more than one possessive pronoun may occur as a pronoun copy, so \REF{ex:4:} is a possible alternative for \REF{ex:4:}:
...
The \textstyleEmphasizedWords{\textsc{head noun}} is either a single or a compound noun.  If the head noun is replaced by a pronoun, it can only take post-modifiers \REF{ex:4:}:
...
A \textstyleEmphasizedWords{\textsc{post-modifying noun phrase}} often expresses qualities that in many European languages would be expressed by true adjectives \REF{ex:4:}, or via adjectivalized \REF{ex:4:} or comitative expressions \REF{ex:4:}.
...
A noun phrase can have one or more \textstyleEmphasizedWords{\textsc{adjective phrases}} as modifiers. The adjective phrase typically consists of an adjective only. If there are more \textstyleAcronymallcaps{\textsc{ap}s} than one, the order is as follows: colour - physical property or human propensity - size/age - value.
...
In recorded texts the maximum number of adjective phrases per a \textstyleAcronymallcaps{NP} is two, but the speakers have no difficulty producing \textstyleAcronymallcaps{NP}s with more \textstyleAcronymallcaps{\textsc{ap}}s \REF{ex:4:}:
...
The \textstyleEmphasizedWords{\textsc{dative pronoun}} (\sectref{sec:3.5.5}) is unusual as a modifier. Semantically it belongs to the noun phrase, marking a possessive relationship, but syntactically it still reflects its origin as a [+human] locative adverbial (\sectref{sec:4.6.1}) of the verb. It is often non-contiguous with the rest of the \textstyleAcronymallcaps{NP}, which can be fronted for as a theme  while the dative pronoun needs to stay in its pre-verbal position \REF{ex:4:}. Other elements that can separate the dative pronoun from the rest of the \textstyleAcronymallcaps{NP} are \textstyleStyleVernacularWordsItalic{me} `not' \REF{ex:4:}, and the free adverbs \textstyleStyleVernacularWordsItalic{muutiw} `only' and \textstyleStyleVernacularWordsItalic{pun} `also'\REF{ex:4:}. 
...
{}
A comitative noun phrase is made up of one or two basic noun phrases plus a comitative postposition or clitic (\sectref{sec:3.12.1}). A comitative pronoun (\sectref{sec:3.5.9}) either by itself or attached to a \textstyleAcronymallcaps{NP} can also form a comitative phrase \REF{ex:4:}. When there is only one overt noun phrase and it  is unmarked for number, the plurality is shown both by the comitative marking and in the verb person marking \REF{ex:4:}. The choice of the comitative marker and the number marking in the verb, when relevant,  reflect whether the noun phrases in the comitative relationship are co-subjects/co-objects of the same verb, or whether one is a dominant member. 
...
With the dual comitative postposition \textstyleStyleVernacularWordsItalic{ikos}  there may be an additive connective \textstyleStyleVernacularWordsItalic{ne} between the two noun phrases \REF{ex:4:}. It seems to be more common with younger speakers. 
...
{}
The head of an adjective phrase (\textstyleAcronymallcaps{\textsc{ap}}) is an adjective. Most commonly it occurs alone, but it can be intensified by an intensity adverb either preceding or following it, or both \REF{ex:4:}.  The negator \textstyleStyleVernacularWordsItalic{marew} `none, no' when following the adjective, negates its quality, thus creating its opposite \REF{ex:4:}.  
...
When the adjective \textstyleStyleVernacularWordsItalic{masia}  `bitter' takes a nominalized verb as its modifier, the meaning of the adjective changes to indicate that one is doing a lot of some action \REF{ex:4:}.
...
The adjective phrase functions as a post-modifier in a noun phrase \REF{ex:4:}, or as a non-verbal predicate \REF{ex:4:}. 
...
{}
A quantifier phrase usually only consists of a quantifier head \REF{ex:4:} (\sectref{sec:3.4}), but it can be modified by a few intensity adverbs \REF{ex:4:} (\sectref{sec:3.9.2}). 
...
A quantifier phrase most commonly functions as a post-modifier in a noun phrase \REF{ex:4:}, but it can also be used as a non-verbal predicate \REF{ex:4:}.
...
The noun phrase indicating a goal normally does not take the locative clitic or any other marking.  The directional verbs are the most common ones used with goal, but other verbs of motion can be used as well \REF{ex:4:}, \REF{ex:4:}. 
...
It is possible to mark the goal with the locative clitic if the goal is mainly important as the location of the following verb. The frequency of this usage for the clitic is low. Example \REF{ex:4:} is repeated below as \REF{ex:4:}: 
...
The dative pronoun is also commonly added when the location is a village or a larger area, which is seen mainly as a setting for the people. In both \REF{ex:4:} and \REF{ex:4:} above it is the \textstyleEmphasizedWords{\textsc{location}} which is in focus, in the former as the closest village to flee to, and in the latter as an object of bombing, so the dative pronoun is not used. In \REF{ex:4:} a certain culturally important place referred to is in the area of the Koran people and considered their property:
...
The noun phrase indicating a path is marked with the instrumental clitic -\textstyleStyleVernacularWordsItalic{iw}, or occasionally with the locative clitic -\textstyleStyleVernacularWordsItalic{pa} \REF{ex:4:}.
...
When a noun phrase acts as a temporal phrase, the locative clitic -\textstyleStyleVernacularWordsItalic{pa} is attached to it \REF{ex:4:}, unless it is followed by another temporal phrase specifying it further or it includes a demonstrative \REF{ex:4:}. If there are several of these temporal noun phrases, their relative order is from the larger time unit to the smaller one. 
...
A temporal phrase may be formed with the instrumental clitic -\textstyleStyleVernacularWordsItalic{iw} (\sectref{sec:3.12.5}) when something takes place repeatedly at the same time of the day. The example \REF{ex:4:} is here repeated as \REF{ex:4:}:
...
The use of locative clitic is restricted almost exclusively to those cases where the instrument is a vehicle \REF{ex:4:}, so they could also be understood as locatives. In other cases it is used rarely \REF{ex:4:}.
