...
\end{exe}
%%%
\citet[716]{bogoras1922} states that the circum-positioned marker of the unbound adjective “sometimes corresponds to the definite\is{species marking!definite} article or designates an object as referred to before.” The unbound adjective, on the other hand, can only occur in absolutive case which is inherently connected to semantic definiteness\is{species marking!definite} \citep[cf.][207, elsewhere]{dunn1999}.
\il{Chukchi|)}
\il{Alutor|)}
\il{Chukotkan languages|)}
\is{head\hyp{}driven agreement|)}
...
\end{exe}
%%%
Examples (\ref{attr demnmlz} through \ref{attr npnmlz}) show that attributive nominalization in Udmurt is a true attribution marking device which is polyfunctional and not restricted to \isi{headless noun phrase}s.
...
Note that attributive \isi{construct state} marking which is cognate with the Ezafe in other Iranian languages occurs in Ossetic as well, but its use is restricted to certain “emphatic”, i.e. \isi{contrastive focus} constructions \cite[467]{thodarson1989}.
\il{Ossetic|)}
\il{Iranian languages|)}
\il{Indo-Iranian languages|)}
