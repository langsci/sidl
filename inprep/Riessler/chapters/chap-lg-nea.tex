...
% \todo[inline]{Habe zur Verdeutlichung Überschriften links ausgerichtet}
}
\end{table}
\il{Swedish|)}
\is{predicative marking|)}
...
% \todo[inline]{Vielleicht wäre es hier besser, die doppelten Einträge auch doppelt zu nennen, um vertikale Linien überflüssig zu machen}
% \todo[inline]{Es sind keine "doppelten Einträge" sondern Synkretismen, d.h. es gibt nur eine morphologische Form für mehrere Kategorien}
} \label{rumanian art}
\end{table}
%%%
\is{species marking!definite|(}
...
\gll 	rabi mi \textbf{en} \textbf{rde\v{c}i} pla\v{s}\v{c}\\
	need.\textsc{3sg} \textsc{1sg.dat} \textsc{indef:m} red:\textsc{attr:m} coat(\textsc{m})\\
\glt (in a store with red coats on display)\\‘I need a RED coat (viz.~one of those red coats).’\footnote{Cf.~the similar construction with concatenative anti\hyp{}construct state agreement marking in Serbian\il{Serbo-Croatian!Serbian} in example \ref{serbian indef}.}
\z
%%%
