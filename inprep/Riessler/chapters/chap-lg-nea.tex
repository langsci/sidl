...
\end{exe}
%%%
\citet[38]{vajda2004} notes that the optional number agreement marking is “a stylistic device used to emphasize the visual impression created by the quality being described”. Probably, this emphasizing construction marks \isi{contrastive focus} of the adjective: ‘large tents’ versus ‘\textsc{large} tents’.
\is{juxtaposition|)}
\is{head\hyp{}driven agreement|)}
...
\gll 	rabi mi \textbf{en} \textbf{rde\v{c}i} pla\v{s}\v{c}\\
	need.\textsc{3sg} \textsc{1sg.dat} \textsc{indef:m} red:\textsc{attr:m} coat(\textsc{m})\\
\glt (in a store with red coats on display)\\‘I need a \textsc{red} coat (viz.~one of those red coats).’\footnote{Cf.~the similar construction with concatenative anti\hyp{}construct state agreement marking in Serbian\il{Serbo-Croatian!Serbian} in example (\ref{serbian indef}).}
\z
%%%
