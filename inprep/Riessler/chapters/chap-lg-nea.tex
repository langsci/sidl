...
%%%
The Samoyedic branch of Uralic can be divided into two subbranches: North Samoyedic comprises the two \ili{Nenets languages} \ili{Forest Nenets} and \ili{Tundra Nenets}, the two \ili{Enets languages} \ili{Forest Enets} and \ili{Tundra Enets} and \ili{Nganasan}. South Samoyedic\il{South Samoyedic languages} has only one subbranch with living languages: Selkup\il{Selkup languages}. The second South Samoyedic subbranch, Sayan\il{Sayan languages}, is now extinct and not considered here.
...
\il{Tundra Nenets|(}
\paragraph*{Juxtaposition and head\hyp{}driven in Tundra Nenets}
Attributive adjectives in the Nenets languages are juxtaposed to the modified noun by default, similar to examples (\ref{enets juxt}) from Forest Enets\il{Forest Enets} and (\ref{hung juxt}) from \ili{Hungarian}. However, it has been noticed in descriptions of Tundra Nenets that optional head\hyp{}driven is possible under certain pragmatic conditions (cf., e.g., \citealt[50, passim]{jalava2013a}; \citealt[151–152, passim]{nikolaeva2014a}; \citealt[544]{salminen1998a}). In (\ref{t nenets agr}), number agreement indicates “some kind of definiteness or concreteness, as opposed to the generic interpretation of the plural NP” \citep[152]{nikolaeva2014a}.
%%%
\begin{exe}
...
\end{exe}
%%%
Siegl's note that “under certain pragmatic functions, apparently definiteness, a kind of number agreement (though not in the dual) seems to be possible” \citep[177{siegl} seems to infer that \ili{Forest Enets} behaves similar to Tundra Nenets. However, neither Siegl's grammar or the other sparse descriptions of Forest Enets provide examples for attributive adjectives undergoing head\hyp{}driven agreement. 
\is{juxtaposition|)}
\il{Tundra Nenets|)}
...
%%%
\ex
\gll	ńaagə-gəj				koru-kəj\\
	good\textsc{-nom:du}	house\textsc{-nom.du}\\
\glt	‘(two) good houses’
%%%
\ex
\gll	ńaаgə-əʔ				koru-ðәʔ\\
	good\textsc{nom:pl}		house\textsc{-nom:pl}\\
\glt	‘(several) good houses’
\end{xlist}
\end{exe}
%%%
Whereas agreement is obligatory for all three number values \textsc{singular}, \textsc{dual} and \textsc{plural}, case agreement is defective. Only in nominative, genitive and accusative, the corresponding case values are inherit from the head noun. If the noun phrase is marked for one of the oblique cases, the adjective is inflected for genitive (\citealt[511]{helimski1998a}; \citealt[157]{wagner-nagy2002a}).
\il{Nganasan|)}
\il{Head-driven agreement|)}
\il{North Samoyedic languages|)}
