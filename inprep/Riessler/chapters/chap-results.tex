...
\item{Anti-construct state} (as in Skolt Saami)\il{Skolt Saami}
...
\item{Head\slash{}driven agreement} (as in Finnish)\il{Finnish}
\item{Apposed head\hyp{}driven agreement} (as in Georgian)\il{Georgian}
...
\end{itemize}
%%%
These parameters are applicable in a typology of general noun phrase syntax (including modifiers which are not adjectives and modifiers which are not simple constituents). Consider Table~\ref{ontologyderived} (derived from Table~\ref{syntaxontology} on page \pageref{syntaxontology}) which includes a phrasally embedded attribute (like the juxtaposed relative clause in Minangkabau, example \ref{juxtrel}), a simple attribute (like the juxtaposed adjective in Komi-Zyrian,\il{Komi-Zyrian} example \ref{juxta}) and an incorporated attribute (like the incorporated possessor in Västerbotten Swedish,\il{Swedish!Västerbotten} example \ref{juxtposs}).
%%%
\begin{exe}
\ex
\begin{xlist}
...
\midrule
%%!!??check formulas$[_{NP} [_{Rel} NP V] N]	&[_{NP} A N]				&[_{NP} _{PSR}N _{PSD}N]$\\
??\\
\lspbottomrule
\end{tabular}
...
Such a development started for instance in \ili{Proto\hyp{}Baltic\slash{}Slavic} and Proto\hyp{}Germanic\il{Proto\hyp{}Germanic} where attributive nominalization arose as a secondary adjective attribution marking device (alongside the original head\hyp{}driven agreement) in contrastive focus constructions and developed further into anti-construct state agreement:
%%%
%!!??check
\begin{exe}
...
Similarly, in Proto\hyp{}Saamic\il{Proto\hyp{}Saamic} attributive nominalization arose as a secondary adjective attribution marking device (in addition to the original juxtaposition) in contrastive focus constructions and developed further into anti-construct state:
%%%
%!!??check
\begin{exe}
