...
\noindent The thesis also provides an extensive appendix including a list of 234 languages sorted by their genealogical affiliation and coded for attested noun phrase types (table starting on page~\pageref{sample1}) as well as a collection of maps illustrating the spread of attested noun phrase types across a world sample of languages (Figure~\ref{WorldMap} and Figure~\ref{WorldMapTyp} on pages~\pageref{WorldMap}–\pageref{WorldMapTyp}), across all northern Eurasian taxa (Figures~\ref{NEMap}–\ref{NAMapTyp} on pages~\pageref{NEMap}–\pageref{NAMapTyp}) and across European languages (Figure~\ref{EUMap} and Figure~\ref{EUMapTyp} on pages~\pageref{EUMap}–\pageref{EUMapTyp}).
...
\ex
\label{dmstate}
{\rm Dependent\hyp{}marking state in \ili{Kildin Saami} (Uralic)}\\(cf.~in more detail page~\pageref{kildin attr.adj.sg})\\
\gll 	ēl'l'-es' 		pērrht.\\
...
\begin{exe}
\ex
{\rm Dependent\hyp{}marking state in \ili{Kildin Saami} (Uralic)}\\(cf.~in more detail page~\pageref{state ap kildin})\\
\gll 	pērht		al'n\\
...
\todo[inline]{was paragraph**{??} the right markup?}
