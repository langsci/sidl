...
\il{Finnish|)}
%%%
Finally, agreement paradigms can be “defective”\is{agreementmarking!defective agreement paradigm} in the sense that certain agreement categories do not show up on all members of the paradigm. In Danish,\il{Danish} for example, gender as an agreement feature is marked on the attributive adjective only in indefinite noun phrases. In noun phrases marked for definite species, the attributive adjective is marked with an invariable definite agreement suffix. Consider (\ref{danish agr ex}) and Table~\ref{danish agr paradigm} with the corresponding paradigm in \S\ref{n-germanic synchr}.\is{species marking!definite}\is{species marking!indefinite}
%%%
\begin{exe}
\ex
\label{danish agr ex}
...
\subsection{Dependent marking attributive state}
...
%Babby (2009: Ch. 2)\cite{babby2009a}
%%%
\begin{exe}
\ex
\langinfo{Russian}{Indo-European}{personal knowledge}
...
% \end{scriptsize}
\caption[Adjective paradigm for \textsc{German}]{Agreement paradigm for the \textsc{German} adjective ‘good’  (‘good man’ \textsc{m}, ‘good woman’ \textsc{f}, ‘good child’ \textsc{n}, ‘good people’ \textsc{pl})\il{German}}
\label{german agr}
\end{sidewaystable}
...
\label{mandarin pred}
\gll	wǒ-de 			shū 		shì 			{\upshape [}\textsubscript{\rm NP} 	xīn 	\textbf{de}{\upshape ]}\\
	\textsc{1sg-nmlz} 	book 	\textsc{cop} 	{} 							new 	\textsc{nmlz}\\
\glt	‘My book is new (i.e., a new one).’
%%%
\ex	\rm{Adjectival adverb}\\
\label{mandarin adv}
\gll	wǒ 			{\upshape [}\textsubscript{\rm NP} 	yánli\textbf{-de}{\upshape ]} 	zébèi 	tā 			le\\
...
%%%
\ea	{\upshape [}\textsubscript{\rm NP} Friedrich {\upshape [}\textsubscript{\rm NP'} \textsubscript{\rm ART}der \textsubscript{\rm A}Große {\upshape ]]}
\z
%%%
...
\langinfo{German}{Indo-European}{personal knowledge}
\begin{xlist}
\ex	Irgendein {\upshape [}Friedrich der Große{\upshape ]}\textsubscript{\textsc{indef.nom}} soll das gesagt haben.
\ex	Dieser {\upshape [}Friedrich der Große{\upshape ]}\textsubscript{\textsc{def.nom}} soll das gesagt haben. 
\ex	Ich sehe mir irgendeinen {\upshape [}Friedrich den Großen{\upshape ]}\textsubscript{\textsc{indef.acc}} an.
\ex	Ich sehe mir diesen {\upshape [}Friedrich den Großen{\upshape ]}\textsubscript{\textsc{def.acc}} an.
\end{xlist}
\end{exe}
%%%
...
\label{linker}
%%
The term \textsc{neutral marking} was introduced by Nichols (\citeyear{nichols1986}) in her typology of head marking versus dependent marking grammar. \textsc{Neutral marking} refers to a marker's locus\is{syntactic locus} neither on-head nor on-dependent. This means that the marker floats in the noun phrase depending on the actual order of constituents. A floating state marker occurs, for instance, in Tagalog.
%%
\begin{exe} 
\ex
...
\end{itemize}
%%%
Table~\ref{tabledefontology1} on page~\pageref{tabledefontology1} summarizes the typology presented in \S\S\ref{juxtaposition}–\ref{linker} and presents short definitions (including bracketed syntactic templates) and an example for each type.\footnote{This overview is derived from the definition file of general noun phrase patterns included in \citet{AUTOTYP-NP}.\is{AUTOTYP}} Note that a lexical head is required only in certain noun phrase types. Note also that the constituent order (e.g., {\upshape [}\textsubscript{\rm NP} A N{\upshape ]} or {\upshape [}\textsubscript{\rm NP} N A{\upshape ]}) and the morpho-phonological fusion of formatives (e.g., (free) {\upshape [}\textsubscript{\rm NP} A \textsc{nmlz}{\upshape ]}, (cumulative) {\upshape [}\textsubscript{\rm AP} A:\textsc{attr:agr}{\upshape ]} or (affixal) {\upshape [}\textsubscript{\rm AP} A-\textsc{attr}{\upshape ]}) is not relevant for the presented ontology.\footnote{The presented ontology is defined by (mostly) morpho-syntactic parameters. But grammatical word-hood could be relevant for definitions of subtypes in the leaves of Figure~\ref{tree ontology}. For instance \textit{head\hyp{}driven agreement} could perhaps be sub-divided into types exhibiting agreement affixes versus grammatical agreement words.}
...
\midrule
Juxt		&Unmarked sequence of constituents; Test: no additional morphemes available in NP&{\upshape [}\textsubscript{\rm NP}[\textsubscript{\rm AP} A{\upshape ]} (N){\upshape ]}					&Juxtaposition			&Komi-Zyrian\il{Komi-Zyrian}\\
\\
Inc		&No additional morphemes available in NP, but dep is syntactic compound; Test: dep cannot occur unbound (headless)&{\upshape [}\textsubscript{\rm NP} A-N{\upshape ]}
		&Incorporation			&Chukchi\il{Chukchi}\\
\\
Constr	&Head-marking formative that only registers presence of dep; Test: formative does not undergo agreement and is not present without head (in predication) or without dep&{\upshape [}\textsubscript{\rm NP}[\textsubscript{\rm AP} A (N:\textsc{attr}){\upshape ]]}			
		&Ezafe				&Kurdish\il{Kurdish languages}\\
\\
Link		&Floating formative (neither ad-head nor ad-dep, but truly ad-phrase) that only registers presence of head-dep relation; Test: formative not present without head (in predication) or without dep&{\upshape [}\textsubscript{\rm NP}[\textsubscript{\rm AP} A] [\textsc{attr}] N{\upshape ]}
...
Type	&Definition&Syntactic&Commonly&Example\\
	&		&dependency&used label&language\\
\midrule
AConstr	&Dep-marking formative that only registers presence of head; Test: formative does not undergo agreement and is not present without head (in predication)&{\upshape [}\textsubscript{\rm NP} [\textsubscript{\rm AP} A:\textsc{attr}{\upshape ]} (N){\upshape ]}
		&Attributive suffix		&Skolt Saami\il{Skolt Saami}\\
\\
Nmlz		&Dep-marking formative that only registers presence of head by projecting full NP; Test: formative does not undergo agreement and is used in focus construction where inflection of the head is duplicated&{\upshape [}\textsubscript{\rm NP}[\textsubscript{\rm NP}[\textsubscript{\rm AP} A\textsc{:nmlz}{\upshape ]]} (N){\upshape ]}
		&Nominalizer			&Udmurt\il{Udmurt}\\
\\
Art		&Subtype of nominalizer that undergoes agreement&{\upshape [}\textsubscript{\rm NP}[\textsubscript{\rm NP}[\textsubscript{\rm AP} A \textsc{nmlz:agr}{\upshape ]]} (N){\upshape ]}
...
Type	&Definition&Syntactic&Commonly&Example\\
	&		&dependency&used label&language\\
\midrule
ACAgr	&Dep-marking formative that registers presence of head and undergoes agreement triggered by the head; Test: not present without head (in predication)&{\upshape [}\textsubscript{\rm NP}[\textsubscript{\rm AP} A\textsc{:attr:agr}{\upshape ]} (N){\upshape ]}
		&Long-form adjective	&Russian\il{Russian}\\
\\
HDAgr	&Dep-marking formative that duplicates morpho-semantic features of the head&{\upshape [}\textsubscript{\rm NP}[\textsubscript{\rm AP} A:\textsc{agr}{\upshape ]} (N){\upshape ]}
		&Agreement suffix		&Finnish\il{Finnish}\\
\\
AHDAgr	&AP marked with HDAgr but projecting a full NP in apposition; Test: AP is used in focus construction where inflection of the head is duplicated, often with reversed constituent order&{\upshape [}\textsubscript{\rm NP}[\textsubscript{\rm NP'} A:\textsc{agr}{\upshape ]} (N){\upshape ]}
		&Appositional agreement	&Georgian\il{Georgian}\\
\\
MHPAgr	&Head-marking formative that duplicates morpho-semantic features of (adjectival) dep by means of possessor agreement in a modifier\hyp{}headed NP&{\upshape [}\textsubscript{\rm NP}[\textsubscript{\rm PSD} A:\textsc{poss:agr}{\upshape ]}(\textsubscript{\rm PSR}N){\upshape ]}
...
% \hline
% \hline
			&\multicolumn{6}{c}{\textbf{Syntactic source}}\is{syntactic source}\rule{0pt}{1.3em}\\

% \hline
% \hline
			&\multicolumn{4}{c}{Government {\upshape [}+GOV{\upshape ]} {\upshape [}±(secondary)AGR{\upshape ]}}&\multicolumn{2}{c}{Agreement {\upshape [}–GOV{\upshape ]} {\upshape [}+(primary)AGR{\upshape ]}}\rule{0pt}{1.3em}\\
			\cline{2-7}
\textbf{Locus}		&\multicolumn{4}{c}{\textbf{Syntactic pattern}}&\multicolumn{2}{c}{\textbf{Syntactic pattern}}\is{syntactic pattern}\is{syntactic locus}\rule{0pt}{1.3em}\\
			\cline{2-7}
		&{\upshape [}±AGR{\upshape ]}	&Embedded	&Non-embedded			&Incorporated	&Embedded&Non-embedded\rule{0pt}{1.3em}\\
% \hline
\midrule
no			&\cellcolor{lightgray}&			&\textbf{Juxtaposition}						&\textbf{Incorporation}			&\cellcolor{lightgray}&\cellcolor{lightgray}\rule{0pt}{1.3em}\\
marking		&\cellcolor{lightgray}&			&{\upshape [}\textsubscript{\rm NP} A (N){\upshape ]}				&{\upshape [}\textsubscript{\rm NP} A *(N){\upshape ]}&\cellcolor{lightgray}&\cellcolor{lightgray}\rule{0pt}{1.3em}\\
% \hline
\hline
\multirow{4}{*}{floating}&\multirow{2}{*}{{\upshape [}–AGR{\upshape ]}	} 			&			&\textbf{Linker}						&			&			&\rule{0pt}{1.3em}\\
	    &&			&{\upshape [}\textsubscript{\rm NP} A {\upshape [}\textsc{attr}{\upshape ]} *(N){\upshape ]}	&			&			&\rule{0pt}{1.3em}\\
 			\cline{2-5}
			&\multirow{2}{*}{{\upshape [}+AGR{\upshape ]}}	&			&							&			&			&\rule{0pt}{1.3em}\\
			&			&			&							&			&			&\rule{0pt}{1.3em}\\
% \hline
\hline
\multirow{4}{*}{dep-marking}			&\multirow{2}{*}{{\upshape [}–AGR{\upshape ]}	} 			&\textbf{Nominalization}&\textbf{Anti-Construct State}			&	&\textbf{Appositional Head-}&\textbf{Head-Driven}\rule{0pt}{1.3em}\\
			& &{\upshape [}\textsubscript{\rm NP}[\textsubscript{\rm NP'} A:\textsc{nmlz}] (N){\upshape ]}&{\upshape [}\textsubscript{\rm NP} A:\textsc{attr} (N){\upshape ]}&&\textbf{Driven Agreement}&\textbf{Agreement}\rule{0pt}{1.3em}\\		
			\cline{2-5}
		&\multirow{2}{*}{{\upshape [}+AGR{\upshape ]}}	&\textbf{Article}		&\textbf{Anti-Construct Agreement}	&			&{\upshape [}\textsubscript{\rm NP} {\upshape [}\textsubscript{\rm NP'} A:\textsc{agr}] (N)]&{\upshape [}\textsubscript{\rm NP} A:\textsc{agr} (N){\upshape ]}\rule{0pt}{1.3em}\\
			&&{\upshape [}\textsubscript{\rm NP}[\textsubscript{\rm NP'} A:\textsc{nmlz:agr}{\upshape ]} (N){\upshape ]}&{\upshape [}\textsubscript{\rm NP} A:\textsc{attr:agr} (N){\upshape ]}&&&\rule{0pt}{1.3em}\\
% \hline
\hline
\multirow{4}{*}{head-marking}			&\multirow{2}{*}{{\upshape [}–AGR{\upshape ]}	} 			&			&\textbf{Construct State}						&			&\textbf{Modifier\hyp{}headed}	&\rule{0pt}{1.3em}\\
 		& 	&			&{\upshape [}\textsubscript{\rm NP} A *(N:\textsc{attr}){\upshape ]}&&\textbf{Possessor Agreement}&\rule{0pt}{1.3em}\\
			\cline{2-5}
 		&\multirow{2}{*}{{\upshape [}+AGR{\upshape ]}}	&			&							&&{\upshape [}\textsubscript{\rm NP}[\textsubscript{\rm PSD} A:\textsc{poss:agr}{\upshape ]} (\textsubscript{\rm PSR}N){\upshape ]}&\rule{0pt}{1.3em}\\
			&			&			&							&&&\rule{0pt}{1.3em}\\
% \hline
\hline
\end{tabular}
