...
\ea \textrm{Attribution: adjective-noun order}\\
\gll	pirka cep\\
	be\_good fish\\
\glt	‘a fine fish’
\ex \textrm{Predication: noun-adjective order}\\
\gll	cep pirka\\
	fish be\_good\\
...
\ea dika\textsuperscript{n} stag\textsuperscript{3} \textrm{‘good person’}	\jambox{\textrm{\textsc{nom:sg}}}
\ex dikaču stega\textsuperscript{n} 								\jambox{\textrm{\textsc{gen:sg}}}
\ex dikaču stagana 											\jambox{\textrm{\textsc{dat:sg}}}
\ex dikaču staga 											\jambox{\textrm{\textsc{erg:sg}}}
\ex dikaču stagie											\jambox{\textrm{\textsc{all:sg}}}
\ex dika\textsuperscript{n} na:x									\jambox{\textrm{\textsc{nom:pl}}}
\ex dikaču ne:xa\textsuperscript{n}								\jambox{\textrm{\textsc{gen:pl}}}
\zl
...
\begin{xlist}
\ex	\textrm{Anti-construct state agreement}
\begin{xlist}
\ex
...
\end{xlist}
\ex \textrm{Predicative agreement}
\begin{xlist}
\ex 
\gll 	\.etot 	dom	vysok\\
...
\begin{xlist}
\ex  \textrm{“short” predicative adjective}\\
\gll on bolen\\
	3\textsc{sg} ill:\textsc{pred:m}\\
\glt	 ‘he is ill’\label{ru-pred-short}
\ex  \textrm{“long” predicative adjective}\\
\gll on bol'nyj\\
...
Friedrich der Große 			\jambox{\textrm{‘Frederick the Great’}}
\z
%%%
...
\end{itemize}
%%%
\tabref{tabledefontology} on page \pageref{tabledefontology} summarizes the typology presented in sections \sectref{juxtaposition}–\sectref{linker} and presents short definitions (including bracketed syntactic templates) and an example for each type.\footnote{This overview is derived from the definition file of general noun phrase patterns included in \citet{AUTOTYP-NP}.} Note that a lexical head is required only in certain noun phrase types. Note also that the constituent order (e.g. $[_\textrm{NP}$ A N$]$ or $[_\textrm{NP}$ N A$]$) and the morpho-phonological fusion of formatives (e.g. (free) $[_\textrm{NP}$ A \textsc{nmlz}$]$, (cumulative) $[_\textrm{AP}$ A:\textsc{attr:agr}$]$ or (affixal) $[_\textrm{AP}$ A-\textsc{attr}$]$) is not relevant for the presented ontology.\footnote{The presented ontology is defined by (mostly) morphosyntactic parameters. But grammatical word-hood could be relevant for definitions of subtypes in the leaves of \figref{tree ontology}. For instance \textit{head-driven agreement} could perhaps be sub-divided into types exhibiting agreement affixes vs. grammatical agreement words.}
...
\midrule
Juxt		&Unmarked sequence of constituents; Test: no additional morphemes available in NP&$[_\textrm{NP}[_\textrm{AP}$ A$]$ (N)$]$					&Juxtaposition			&Komi	\\
...
marking		&/\textbackslash&			&$[_\textrm{NP}$ A (N)$]$				&$[_\textrm{NP}$ A *(N)$]$&/\textbackslash&/\textbackslash\\
\hline
\hline
			& 			&			&\textbf{Linker}						&			&			&\\
...
dep-			&$[$–AGR$]$	&$[_\textrm{NP}[_\textrm{NP'}$ A:\textsc{nmlz}] (N)$]$&$[_\textrm{NP}$ A:\textsc{attr} (N)$]$&&\textbf{Driven Agreement}&\textbf{Agreement}\\		
			\cline{2-5}
marking		&$[$+AGR$]$	&\textbf{Article}		&\textbf{Anti-Construct Agreement}	&			&[$_{NP}$ [$_{NP'}$ A \textsc{agr}] (N)]&$[_\textrm{NP}$ A \textsc{agr} (N)$]$\\
